...
# Copyright 2002 to 2005 by Éric Beaudoin <beaudoer@videotron.ca>.
...
# This script reads a PCGEN .lst and reformat it to follow the directions
# given by Tir and other list monkeys.
...
# Version information
my $CVS_id = '$Id: prettylst.pl,v 1.355 2005/01/19 01:57:53 Eric Exp $';
my ($CVS_build,$CVS_date) = ($CVS_id =~ m!v \d\.(\d+) (\d{4}/\d\d/\d\d)!);
$CVS_build += 100;
$CVS_date =~ tr!/!.!;
my $VERSION = "1.34 (build $CVS_build)";
my $VERSION_DATE = $CVS_date;
my ($SCRIPTNAME) = ($0 =~ m!([^/\\]*)$!);
my $LongVersion = "$SCRIPTNAME version: $VERSION -- $VERSION_DATE";
...
sub mywarn($$;$);
sub ewarn($$$;$);
sub set_mywarn_filename($);
sub set_mywarn_header($);
...
# Valid filetype are the only ones that will be parse
...
my %validgames = map { $_ => 1 }
      (
        '3e',                         # 5.3.13 new game mode
        '35e',                        # 5.3.13 new game mode
        'Arcana Unearthed',           # [ 829380 ] New Game Mode
        'Modern',                     # 4.1.6, new game mode
        'Sidewinder',
        'Spycraft',
        'Xcrawl',                     # [ 1039028 ] [PCC]New Xcrawl Game Mode

        # CMP game modes
        'd20Modern',
#        'DnD',                       # 5.3.13 deprecated
        'DnD_v30e',
        'DnD_v35e',
        'DnD_v3.5e_VPWP',
        'DnD_v35e_Dragonlance',
...
my $opt_outputpath = "";
my $opt_report = 0;
my $opt_test = 0;         # Internal, for tests only
my $opt_xcheck = 1;       # Perform cross-check validation

# Command line options
my %cl_options = (
      convert         =>  "",     # Activate a standard convertion
      gamemode        => "",      # GAMEMODE filter for the PCC files
...
if(scalar @ARGV)
{
    &GetOptions(
        ...
        "help|h|?"          => \$opt_help,
        "htmlhelp"          => \$opt_htmlhelp,
        "inputpath|i=s"     => \$opt_inputpath,
        "man"               => \$opt_man,
        "missingheader|mh"  => \$cl_options{missing_header},
        "outputerror|e=s"   => \$opt_outputerror,
        "outputpath|o=s"    => \$opt_outputpath,
        "report|r"          => \$opt_report,
        "test"              => \$opt_test,
        "xcheck|x"          => \$opt_xcheck,
...
{
  if($cl_options{convert} eq "pcgen5713")
  {
...
if(!$opt_inputpath && !$opt_filetype &&
   !($opt_man || $opt_htmlhelp))
{
...
unless($cl_options{basepath})
{
  $cl_options{basepath} = $opt_inputpath;
}
...
if ($opt_help or $Getopt::Long::error)
{
    Pod::Usage::pod2usage(
              {-msg => $error_message,
               -exitval => 1,
...
if ($opt_htmlhelp)
{
  generate_css("$0.css") if !-e "$0.css";

  Pod::Html::pod2html("--infile=$0",
           "--outfile=$0.html",
           "--css=$0.css",
           "--title=$0 -- Reformat the PCGEN .lst files",
           "--header");
...
my $source_curent_file  = "" if $conversion_enable{'SOURCE line replacement'};
...
my %tag_fix_value = (
      ...
      FREE              => { YES => 1, NO => 1 },
      KEYSTAT           => { STR => 1, DEX => 1, CON => 1, INT => 1, WIS => 1, CHA => 1 },
...
use constant    DEBUG       => 1;     # Normal message + debug message for the programmer
use constant    INFO        => 2;     # Everything including deprecations message
use constant    NOTICE      => 3;     # No deprecations (default)
use constant    WARNING     => 4;     # PCGEN will prabably not work properly
use constant    ERR         => 5;     # PCGEN will not work properly or the
                                      # script is foobar
...
my %master_file_type = (
     ...
     BIOSET     => [
                  { Linetype          => 'SOURCE',
                    ...
                    Format            => LINE,
                    Header            => NO_HEADER,
                    Sep               => '|',         # use | instead of [tab] to split
                    SepRegEx          => qr(\|),
                  },
...
map { my $tag = $_; $tag =~ s/:\*$//; $PRE_Tags{$tag} = 1 } @PRE_Tags;
...
my %master_order = (
  ...
  "CLASS"           => [
    ...
    'PROHIBITED',                 # [ 974693 ] PROHIBITED class tag
    'LEVELSPERFEAT',
    'VFEAT',
    'MULTIPREREQS',
#    'SPECIALS',
    ...
    'AUTO:SHIELDPROF',
    'AUTO:WEAPONPROF:*',
    'ADDDOMAINS',             # [ 988630 ] ADDDOMAINS tag missing
    'WEAPONAUTO',
    'BONUS:CASTERLEVEL:*',    # [ 1004081 ] Missing global BONUS:CASTERLEVEL
    'BONUS:CHECKS:*',
    'BONUS:COMBAT:*',
    'BONUS:DAMAGE:*',
    'BONUS:DEFINE:*',
    ...
    'BONUS:EQMWEAPON:*',
    'BONUS:ESIZE:*',
    'BONUS:FEAT:*',           # [ 1093134 ] BONUS:FEAT|POOL|x
    'BONUS:HD:*',
    'BONUS:HP:*',
    'BONUS:LANGUAGES:*',
    'BONUS:MISC:*',
#    'BONUS:MOVE:*',          # 4.3.10, deprecated, use BONUS:MOVEADD or BONUS:POSTMOVEADD
    'BONUS:MOVEADD:*',        # 4.3.10, new tag
    'BONUS:MOVEMULT:*',       # 4.3.10, new tag
    'BONUS:POSTMOVEADD:*',    # 4.3.10, new tag
    'BONUS:POSTRANGEADD:*',
    'BONUS:PCLEVEL:*',
    'BONUS:RANGEADD:*',
    'BONUS:RANGEMULT:*',
    ...
    'SKILLLIST',
    'SPELLLIST',
    'TEMPLATE',               # 4.3.11, never used before
    @PRE_Tags,
    'PRERACETYPE',            # Only in CLASS
    '!PRERACETYPE',           # Only in CLASS
    'STARTSKILLPTS',
    'MODTOSKILLS',
    'CSKILL:.CLEAR',
    'CSKILL',
    'CCSKILL',
    'MONSKILL',               # [ 1097487 ] MONSKILL in class.lst
    'MONNONSKILLHD:*',        # [ 1048297 ] New Tag: MONNONSKILLHD
    'SPELLLEVEL:CLASS',       # [ 813504 ] SPELLLEVEL:DOMAIN in domains.lst
    'SPELLLEVEL:DOMAIN',      # [ 813504 ] SPELLLEVEL:DOMAIN in domains.lst
  ],
  ...
  "CLASS Level"     => [
    ...
    'DR',
    'DOMAIN:*',
    'DEITY',                  # [ 976474 ] DEITY tag is missing from CLASS Level
    'SA:.CLEAR:*',
    'SA:*',
    'BONUS:VAR:*',
    'BONUS:CASTERLEVEL:*',    # [ 1004081 ] Missing global BONUS:CASTERLEVEL
...
#    'BONUS:MOVE:*',          # 4.3.10, deprecated, use BONUS:MOVEADD or BONUS:POSTMOVEADD
    ...
    'BONUS:RANGEADD:*',
    'BONUS:RANGEMULT:*',
#    'BONUS:REPUTATION:*',           # Should be depreciated.
    'BONUS:SCHOOL:*',
    'BONUS:SIZEMOD:*',
    'BONUS:SKILL:*',
    'BONUS:SKILLPOINTS:*',
    ...
    'BONUS:UDAM:*',
    'BONUS:VISION:*',
    'BONUS:WEAPON:*',             #This should only be in equip and equipmod
    'BONUS:WEAPONPROF:*',
    'DEFINE:*',
    'CSKILL:.CLEAR',
    'CSKILL',
    ...
    'ADD:DOMAIN',
    'ADD:FEAT:*',
#    'ADD:FORCEPOINT',            # This should also be gone
    'ADD:Language:*',
    'ADD:LIST:*',
    'ADD:SPECIAL:*',
    'ADD:VFEAT',
    'ADD:WEAPONBONUS',
    'LANGBONUS',              # [ 976475 ] Missing LANGBONUS tag in CLASS Level
    'CHOOSE',
    'EXCHANGELEVEL',
    'FEAT',                   #Should be deprecieated (FEATAUTO And ADD:FEAT() now...)
    'SPECIALS',
    'SPELL',
    'SPELLS:*',
    'TEMPLATE',
    'VFEAT',
#    'AUTO:*',                # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:ARMORPROF',         # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:EQUIP',             # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:FEAT',              # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:SHIELDPROF',        # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:WEAPONPROF:*',        # [ 832171 ] AUTO:* needs to be separate tags
    'ADDDOMAINS',
    'QUALIFY',
    'WEAPONAUTO',
    'WEAPONBONUS',
    ...
    'SPELLLEVEL:DOMAIN',      # [ 813504 ] SPELLLEVEL:DOMAIN in domains.lst
    'SPELLLIST',
    'NATURALATTACKS',         # [ 832139 ] CLASS Level: missing NATURALATTACKS
  ],
...
#    'BONUS:MOVE:*',          # 4.3.10, deprecated, use BONUS:MOVEADD or BONUS:POSTMOVEADD
    ...
    'BONUS:VAR:*',
    'BONUS:WEAPONPROF:*',
    'SWITCHRACE:*',               # 4.3.9, new for companionmod
  ],
...
#    'BONUS:MOVE:*',          # 4.3.10, deprecated, use BONUS:MOVEADD or BONUS:POSTMOVEADD
    ...
    'BONUS:WEAPON:*',
    'BONUS:WEAPONPROF:*',
    'DEFINE:*',                 # 5.3.1, never used before
    'SR',
    'DR',
    'VFEAT',
    'SA:.CLEAR',
  ...
  "DOMAIN"          => [
    ...
    'FEAT',
    'VFEAT',
    'ADD:FEAT:*',             # [ 823764 ] ADD:FEAT in domain list
#    'AUTO:*',                # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:ARMORPROF',         # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:EQUIP',             # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:FEAT',              # [ 832171 ] AUTO:* needs to be separate tags
...
#    'BONUS:MOVE:*',          # 4.3.10, deprecated, use BONUS:MOVEADD or BONUS:POSTMOVEADD
  ...
  "EQUIPMENT"       => [
    ...
    'DEFINE:*',
    'ACCHECK',
#    'ATTACKS',           # 4.3.2, removed, use BONUS:COMBAT|ATTACKS|x instead
    'BASEITEM',
    'BASEQTY',
    'CHOOSE',
    'CRITMULT',
...
#    'MOVE',
    ...
    'REACH',
    'SIZE',
    'SOURCEPAGE:.CLEAR',  # 5.xxx, never used before
    'SOURCEPAGE',
    'SOURCEWEB',
    'SPELLFAILURE',
    'ADD:DOMAIN',
    ...
    'SR',
    'DR',
    'SPELL:*',                # [ 823221 ] SPELL multiple time on equipment
    'SPELLS:*',               # [ 823221 ] SPELL multiple time on equipment
    'BONUS:CASTERLEVEL:*',    # [ 1004081 ] Missing global BONUS:CASTERLEVEL
    'BONUS:CHECKS:*',
    'BONUS:COMBAT:*',
    'BONUS:DAMAGE:*',
...
#    'BONUS:MOVE:*',          # 4.3.10, deprecated, use BONUS:MOVEADD or BONUS:POSTMOVEADD
    ...
    'SPELLS:*',
    'RATEOFFIRE',
#    'PREFEAT',          # Don't want to comment for this until it works - Talk to Hugo
  ],
  ...
  "FEAT"            => [
    ...
    'MULT',
    'CHOOSE',
    'TEMPLATE',               # New for 5.6.1
    'MOVE',                 # 5.xxx, new tag
    'MOVECLONE',            # 5.xxx, new tag
#    'AUTO:*',                # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:ARMORPROF',         # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:EQUIP',             # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:FEAT',              # [ 832171 ] AUTO:* needs to be separate tags
    ...
    'ADD:CLASSSKILLS',
    'ADD:FAVOREDCLASS',
    'ADD:FEAT:*',             # [ 804091 ] ADD:FEAT warning
    'ADD:FORCEPOINT',
    'ADD:Language',           # [ 1005651 ] ADD:Language in a feat file
    'ADD:LIST',
    'ADD:SPECIAL',
    'ADD:SPELLCASTER',        # [ 1005661 ] ADD:SPELLCASTER in feat .lst
    'ADD:WEAPONPROFS',
    'ADDSPELLLEVEL',
    'LANGAUTO',
    'BONUS:CASTERLEVEL:*',    # [ 1004081 ] Missing global BONUS:CASTERLEVEL
...
#    'BONUS:MOVE:*',          # 4.3.10, deprecated, use BONUS:MOVEADD or BONUS:POSTMOVEADD
    ...
    'SOURCEPAGE',
    'SOURCEWEB',
    'NATURALATTACKS',         # 5.xxx, never used before
    'BENEFIT',
    'TEMPDESC',               # PREAPPLY description
  ],
  ...
  "KIT LANGAUTO"        => [
    'LANGAUTO',               # [ 829335 ] New LANGAUTO line type for KIT files
  ],
  ...
  "KIT STARTPACK"   => [
    'STARTPACK',
    'EQUIPBUY',               # 5.3.1, new tag
    'EQUIPSELL',              # 5.3.1, new tag
    @PRE_Tags,
    'SOURCEPAGE',
  ],
...
#    'OUTPUTNAME',
  ...
  "MASTERBONUSRACE" => [    # 4.3.9, new linetype
    '000MasterBonusRace',
    'TYPE',
    'BONUS:CASTERLEVEL:*',    # [ 1004081 ] Missing global BONUS:CASTERLEVEL
    'BONUS:CHECKS:*',
    'BONUS:COMBAT:*',         # [ 1066352 ] BONUS:COMBAT|INITIATIVE on MASTERBONUSRACE line
    'BONUS:DC',               # [ 813337 ] BONUS:DC
    'BONUS:FEAT:*',           # [ 1093134 ] BONUS:FEAT|POOL|x
    'BONUS:MOVEADD:*',        # 4.3.10, new tag
    'BONUS:HP',
    ...
    'BONUS:STAT:*',
    'BONUS:UDAM:*',
    'VFEAT',                # 4.3.10, new tag
    'SA',
  ],

  "PCC"             => [
    'CAMPAIGN',
#    'GAME',              # 5.3.3, deprecated, use GAMEMODE
    'GAMEMODE',           # 5.3.3, replaced GAME
    'GENRE',              # 5.xxx, new tag
    'BOOKTYPE',           # 4.3.6, new tag
    'PUBNAMELONG',        # 5.xxx, new tag
    'PUBNAMESHORT',       # 5.xxx, new tag
    'PUBNAMEWEB',         # 5.xxx, new tag
    'SETTING',            # 5.xxx, new tag
    'TYPE',               # 4.3.6, deprecated, use BOOKTYPE and SOURCExxx
    'SOURCELONG',
    'SOURCESHORT',
    'SOURCEWEB',
    'COPYRIGHT',
...
#    'BONUS:MOVE:*',          # 4.3.10, deprecated, use BONUS:MOVEADD or BONUS:POSTMOVEADD
    ...
    'BONUS:SCHOOL:*',
    'BONUS:SIZEMOD:*',
    'BONUS:SLOTS:*',          # [ 1005655 ] BONUS:SLOTS in race files
    'BONUS:SKILL:*',
    'BONUS:SKILLPOINTS:*',
    'BONUS:SKILLPOOL:*',
    'BONUS:SKILLRANK:*',
    ...
    'CSKILL',
    'CCSKILL',
    'MONCSKILL',            # [ 813333 ] MONCSKILL and MONCCSKILL in race.lst
    'MONCCSKILL',           # [ 813333 ] MONCSKILL and MONCCSKILL in race.lst
#    'AUTO:*',                # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:ARMORPROF',         # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:EQUIP',             # [ 832171 ] AUTO:* needs to be separate tags
    'AUTO:FEAT',              # [ 832171 ] AUTO:* needs to be separate tags
    ...
    'CHOOSE',
    'ADD:FEAT:*',             # [ 804091 ] ADD:FEAT warning
    'ADD:SPELLCASTER:*',      # [ 1004893 ] ADD:SPELLCASTER is valid in RACE
    'REGION',                 # 5.3.1, new tag
    'SPELLLEVEL:DOMAIN',      # [ 813504 ] SPELLLEVEL:DOMAIN in domains.lst
  ],
...
#    'BONUS:MOVE:*',          # 4.3.10, deprecated, use BONUS:MOVEADD or BONUS:POSTMOVEADD
    ...
    'BONUS:WEAPONPROF:*',
    'REQ',
    'SA',                     # [ 1006371 ] SA tag in Skill .lst
    'TEMPDESC',               # PREAPPLY description
  ],
  ...
  "SPELL"           => [
    ...
    'XPCOST',
    @PRE_Tags,
    'DESCISPI',               # [ 824975 ] spells.lst - DESCISPI:[YES/NO]
    'DESC',
    'BONUS:CASTERLEVEL:*',    # [ 1004081 ] Missing global BONUS:CASTERLEVEL
    'BONUS:CHECKS',
    'BONUS:COMBAT:*',           # 5.3.3, never used before
    'BONUS:DAMAGE:*',
    'BONUS:FEAT:*',           # [ 1093134 ] BONUS:FEAT|POOL|x
    'BONUS:HP',
    'BONUS:MOVEADD',            # [ 1066355 ] BONUS tags in spells.lst
    'BONUS:MOVEMULT',           # [ 1066355 ] BONUS tags in spells.lst
    'BONUS:POSTMOVEADD',
    'BONUS:SIZEMOD',            # [ 1066355 ] BONUS tags in spells.lst
    'BONUS:SKILL:*',
    'BONUS:STAT:*',
    'BONUS:UDAM:*',
    'BONUS:VAR',
  ...
  "SUBCLASS"        => [
    'SUBCLASS',
    'NAMEISPI',                       # [ 882797 ] SUBCLASS -- NAMEISPI: tag
    'OUTPUTNAME',
    'HD',                             # [ 817399 ] Tags usable in SUBCLASS
    'ABB',
    'COST',
    'PROHIBITCOST',
    'CHOICE',
    ...
    'SPELLTYPE',
    'BONUS:CASTERLEVEL:*',    # [ 1004081 ] Missing global BONUS:CASTERLEVEL
    'BONUS:CHECKS:*',                 # [ 817399 ] Tags usable in SUBCLASS
    'BONUS:COMBAT:*',                 # [ 817399 ] Tags usable in SUBCLASS
    'BONUS:DC:*',                     # 4.3.8, newly used
    'BONUS:FEAT:*',           # [ 1093134 ] BONUS:FEAT|POOL|x
    'BONUS:SKILL:*',                  # [ 817399 ] Tags usable in SUBCLASS
    'BONUS:UDAM:*',
    'BONUS:VAR:*',
    'KNOWNSPELLSFROMSPECIALTY',
    'PROHIBITED',                     # [ 974693 ] PROHIBITED class tag
  ...
  "SUBCLASSLEVEL"   => [
    ...
    'UMULT',
    'ADD:SPELLCASTER:*',
    'SPELLLEVEL:CLASS',       # [ 882799 ] SUBCLASSLEVEL -- add SPELLLEVEL:CLASS tag
    'CAST',
    'KNOWN',
    'SPECIALTYKNOWN',
    'KNOWNSPELLS',
...
#    'ADD:FORCEPOINT',            # This should also be gone
    ...
    'EXCHANGELEVEL',
#    'FEAT',                   #Should be deprecieated (FEATAUTO And ADD:FEAT() now...)
#    'INTMODTOSKILLS',         #Reason this is on the class level line?
    'SPECIALS',
    'SPELL',
    'SPELLS:*',
    'TEMPLATE',
...
#    'BONUS:MOVE:*',          # 4.3.10, deprecated, use BONUS:MOVEADD or BONUS:POSTMOVEADD
    ...
    'CSKILL',
    'ADD:CLASSSKILLS',
    'ADD:FEAT:*',           # 4.3.3, never used before
    'ADD:VFEAT',
    'FAVOREDCLASS',
    'FEAT:*',
    'VFEAT',
#    'AUTO:*',                # [ 832171 ] AUTO:* needs to be separate tags
    ...
    'HEIGHT',
    'KIT',
    'LANGAUTO:.CLEAR',      # 4.3.2, new for Kalamar, not sure if good
    'LANGAUTO:*',
    'LANGBONUS',
    'MOVE',
    'MOVEA',
  ...
  "WEAPONPROF" =>   [
    ...
    'OUTPUTNAME',
    'TYPE',
#    'SIZE',                  # [ 845853 ] SIZE is no longer valid in the weaponprof files
    'HANDS',
    @PRE_Tags,
    'SOURCEPAGE',
    'SOURCEWEB',
...
#my %token_ADD_tag = (      # See parse_ADD_tag
#    'ADD:CLASSSKILLS'       => 1,
#    'ADD:DOMAIN'            => 1,     # New for 4.2.2
#    'ADD:FAVOREDCLASS'      => 1,
#    'ADD:FEAT'              => 1,
#    'ADD:FORCEPOINT'        => 1,
#    'ADD:INIT'              => 1,
...
my @token_BONUS_tag = (
    ...
    'MISC',
    'MOVE',
    'MOVEADD',            # 4.3.10, new tag
    'MOVEMULT',           # [ 1005658 ] BONUS:MOVEMULT
    'POSTRANGEADD',
    'POSTMOVEADD',        # 4.3.10, new tag
    'PCLEVEL',
    'RANGEADD',
    'RANGEMULT',
    'REPUTATION',
    'SIZEMOD',            # 5.xxx, never used before
    'SKILL',
    'SKILLPOINTS',
    'SKILLPOOL',
    'SKILLRANK',
    'SLOTS',              # 4.1.4, new bonus type
    'SPELL',              # 4.3.3, was missing
    'SPELLCAST',
    'SPELLCASTMULT',
    'SPELLKNOWN',
    'VISION',
...
# Let's Hash the thing to speed things up
my %token_BONUS_tag = map { $_ => 1 } @token_BONUS_tag;

# List of types that are valid in BONUS:SLOTS
my @token_BONUS_SLOTS_types = (
    "AMULET",
    "BELT",
    "BOOT",
    "BRACER",
...
my %token_BONUS_SLOTS_types = map { $_ => 1 } @token_BONUS_SLOTS_types;
...
# Let's Hash the thing to speed things up
my %token_CHOOSE_tag = map { $_ => 1 } @token_CHOOSE_tag;
...
#    'ATTACKS'             => "Number of Attacks",
    ...
    'MONSKILL'            => "Monster Initial Skill Points",   # [ 1097487 ] MONSKILL in class.lst
    'MOVE'                => "Move",
    'MULT'                => "Multiple?",
    ...
    'PREBASESIZEGT'       => "PREBASESIZEGT",             # [ 823194 ] PREBASESIZExxx tags
    'PREBASESIZEGTEQ'     => "Minimum Size",              # [ 823194 ] PREBASESIZExxx tags
    'PREBASESIZELT'       => "PREBASESIZELT",             # [ 823194 ] PREBASESIZExxx tags
    'PREBASESIZELTEQ'     => "Maximum Size",              # [ 823194 ] PREBASESIZExxx tags
    'PREBASESIZENEQ'      => "PREBASESIZENEQ",            # [ 823194 ] PREBASESIZExxx tags
    ...
    'VISIBLE'             => "Visible",
    'VISION'              => "Vision",
    'WEAPONAUTO'          => "Automatic Weapon Prof.",
  ...
  'RACE' => {
    ...
    'MONCSKILL'           => "Racial HD Class Skills",          # [ 813333 ] MONCSKILL and MONCCSKILL in race.lst
    'MONCCSKILL'          => "Racial HD Cross-class Skills",    # [ 813333 ] MONCSKILL and MONCCSKILL in race.lst
  },
...
if($opt_outputerror)
{
  open NEWSTDERR, ">$opt_outputerror" or die "Can't create $opt_outputerror: $!";
  *STDERR = *NEWSTDERR;
}
...
if($opt_inputpath)
{
  ...
  if($opt_outputpath && !-d $opt_outputpath)
  {
    $error_message = "\nThe directory $opt_outputpath does not exists.";
  ...
  #################################################
  # We populate %valid_tags for for all file types.

  foreach my $filetype (keys %master_order)
  {
    foreach my $tag (@{$master_order{$filetype}})
    {
      if($tag =~ /(.*):\*$/)
      {
        $tag = $1;
        $master_mult{$filetype}{$tag} = 1;
      }

      if(exists $valid_tags{$filetype}{$tag})
      {
        die "Tag $tag found more then once for $filetype";
      }
      else
      {
        $valid_tags{$filetype}{$tag} =1;
      }
    }
  }
  ...
  if($conversion_enable{'Export lists'})
  {
    ...
    open CLASS, ">class.csv" or die "Can't open class.csv:$!";
    print CLASS qq("Class Name","Line","Filename"\n);

    # Will hold the list of all domains found in DOMAIN filetypes
    open DOMAIN, ">domain.csv" or die "Can't open domain.csv:$!";
    print DOMAIN qq("Domain Name","Line","Filename"\n);

    # Will hold the list of all equipements found in EQUIPMENT filetypes
    open EQUIPMENT, ">equipment.csv" or die "Can't open equipment.csv:$!";
    print EQUIPMENT qq("Equipment Name","Output Name","Line","Filename"\n);

    # Will hold the list of all equipmod entries found in EQUIPMOD filetypes
    open EQUIPMOD, ">equipmod.csv" or die "Can't open equipmod.csv:$!";
    print EQUIPMOD qq("Equipmod Name","Key","Type","Line","Filename"\n);

    # Will hold the list of all equipements found in EQUIPMENT filetypes
    open FEAT, ">feat.csv" or die "Can't open feat.csv:$!";
    print FEAT qq("Feat Name","Line","Filename"\n);

    # Will hold the list of all language found in LANGUAGE linetypes
    open LANGUAGE, ">language.csv" or die "Can't open language.csv:$!";
    print LANGUAGE qq("Language Name","Line","Filename"\n);

    # Will hold the list of all spells found in SPELL filetypes
    open SKILL, ">skill.csv" or die "Can't open skill.csv:$!";
    print SKILL qq("Skill Name","Line","Filename"\n);

    # Will hold the list of all spells found in SPELL filetypes
    open SPELL, ">spell.csv" or die "Can't open spell.csv:$!";
    print SPELL qq("Spell Name","Source Page","Line","Filename"\n);

    # Will hold the list of all variables found in DEFINE tags
    if($opt_xcheck)
    {
      open VARIABLE, ">variable.csv" or die "Can't open variable.csv:$!";
      print VARIABLE qq("Var Name","Line","Filename"\n);
    }

    # Will hold the list of all PCC files found
    open PCC, ">pcc.csv" or die "Can't open pcc.csv:$!";
    print PCC qq("SOURCELONG","SOURCESHORT","GAMEMODE","Full Path"\n);
  ...
  # convertion to work
  $opt_xcheck = 1 if $conversion_enable{'CLASSSPELL convertion to SPELL'};
...
#    return if $File::Find::dir =~ /cvs$/i || $File::Find::dir =~ /customsources$/i || $File::Fine:dir =~ /gamemodes/i;
  ...
  File::Find::find(\&mywanted, $opt_inputpath);
  ...
  foreach my $pccfile (sort @filelist)
  {
    open PCCFILE, "$pccfile" or die "Cannot open $pccfile: $!";

    my $currentbasedir = File::Basename::dirname($pccfile);  # Needed to find the full path

    my $mustwrite   = 0;
    my $BOOKTYPEfound   = 0;
    my $GAMEMODEfound   = "";                 # For the PCC export list
    my $SOURCELONGfound = "";                 #
    my $SOURCESHORTfound = "";                #
    my $LSTfound    = 0;
    my $CVSTagfound = 0;
    ...
    while(<PCCFILE>)
    {
      ...
      chomp;
      $mustwrite += s/[\x0d\x0a]//g;          # Remove the real and weird CR-LF
      $mustwrite += s/\s+$//;                 # Remove the tralling white spaces

      push @pcclines, $_;

      my ($tag, $value) = parse_tag $_, "PCC", $pccfile, $.;

      if($tag && "$tag:$value" ne $pcclines[-1])
      {
        # The parse_tag function modified the values.
        $mustwrite = 1;
        $pcclines[-1] = "$tag:$value";
      }
      ...
      if($tag)
      {
        if($validfiletype{$tag})
        {
          ...
          $value =~ s/^([^|]*).*/$1/;
          my $lstfile = findfullpath($value,$currentbasedir,$cl_options{basepath});
          $files_to_parse{$lstfile} = $tag;

          # Check to see if the file exists
          if(!-e $lstfile)
          {
            $filelist_missing{$lstfile} = [ $pccfile, $. ];
                ...
                ($tag eq 'CLASSSPELL' || $tag eq 'CLASS' || $tag eq 'DOMAIN'))
          {
            ...
            {
              push @pcclines, "#" . pop @pcclines;
              $mustwrite = 1;

              mywarn qq(=> Commenting out "$pcclines[$#pcclines]"),
                $pccfile, $.;
            }
          }
                ...
                $tag eq 'CLASSSKILL')
          {
            ...
            mywarn qq(=> Commenting out "$pcclines[$#pcclines]"), $pccfile, $.;
...
#          ($lstfile) = ($lstfile =~ m{/([^/]+)$});
        ...
        elsif($tag =~ /^\#/)
        {
          ...
          $CVSTagfound = 1 if /^\#.*CVS.*Revision/i;
        }
        elsif($valid_tags{'PCC'}{$tag})
        {
          ...
          if($conversion_enable{'SOURCE line replacement'} &&
             ($tag eq 'SOURCELONG'  ||
              ...
              $tag eq 'SOURCEWEB'))
          {
            my $path = File::Basename::dirname($pccfile);
            if(exists $source_tags{$path}{$tag} &&
               $path !~ /custom|altpcc/i)
            {
              mywarn "$tag already found for $path", $pccfile, $.;
            }
            else
            {
            ...
            {
              $SOURCELONGfound = $value;
            }
            elsif($tag eq 'SOURCESHORT')
            {
              $SOURCESHORTfound = $value;
            }
          }
          elsif($tag eq 'GAMEMODE')
          {
            ...
            # and match the filer.
            $GAMEMODEfound = $value;       # The GAMEMODE tag we found
            my @modes = split '\|', $value;
            my $gamemode_regex = $cl_options{gamemode}
                               ? qr(^(?:$cl_options{gamemode})$)i
                               : '.';
            my $valid_game_mode = $cl_options{gamemode}
                                ? 0 : 1;

            # First the filter is applied
            for my $mode (@modes)
            {
              $valid_game_mode = 1 if $mode =~ /$gamemode_regex/
            }
            ...
            if($valid_game_mode)
            {
              for my $mode (@modes)
              {
                mywarn "Invalid GAMEMODE \"$mode\" in \"$_\"",$pccfile,$. unless $validgames{$mode};
              }
            }
          ...
          {
            # Found a TYPE tag
            #mywarn "TYPE should be Publisher.Format.Setting, something is wrong with \"$_\"",
            #       $pccfile, $. if 2 != tr!.!.!;
            $BOOKTYPEfound = 1;
          }
          elsif($tag eq 'GAME' && $conversion_enable{'PCC:GAME to GAMEMODE'})
          {
            ...
            mywarn qq(=> Replacing "$tag:$value" by "GAMEMODE:$value"),
              $pccfile, $.;
            $GAMEMODEfound = $value;
            $mustwrite = 1;
          }
      ...
      {
        mywarn("HTML file detected. Maybe you had a problem with your CSV checkout.\n",$pccfile);
        $mustwrite = 0;
        last;
      }
    }

    close PCCFILE or die "Cannot close $pccfile: $!";

    mywarn "!! No SPELL file found, create one.", $pccfile
      if($conversion_enable{'CLASSSPELL convertion to SPELL'} &&
         $found_filetype{'CLASSSPELL'} &&
         !$found_filetype{'SPELL'});

    mywarn "!! No CLASS file found, create one.", $pccfile
      if($conversion_enable{'CLASSSKILL convertion to CLASS'} &&
         $found_filetype{'CLASSSKILL'} &&
         !$found_filetype{'CLASS'});

    mywarn "No BOOKTYPE tag found", $pccfile if !$BOOKTYPEfound && $LSTfound;
    mywarn "No GAMEMODE tag found", $pccfile unless $GAMEMODEfound;
    print PCC qq("$SOURCELONGfound","$SOURCESHORTfound","$GAMEMODEfound","$pccfile"\n)
      if $GAMEMODEfound && $cl_options{exportlist};

    # Do we copy the .PCC???
    if($opt_outputpath && ($mustwrite || !$CVSTagfound) && $writefiletype{"PCC"})
    {
      my $newfile = $pccfile;
      $newfile =~ s/$opt_inputpath/$opt_outputpath/i;

      # Create the subdirectory if needed
      create_dir(File::Basename::dirname($newfile),$opt_outputpath);

      open NEWFILE, ">$newfile" or die "Cannot create $newfile: $!";

      # We keep track of the files we modify
      push @modified_files, $pccfile;

      # We add a CVS revision number is not present
      print NEWFILE "# CVS \$Revision\$ \$Author\$ -- $today -- reformated by $SCRIPTNAME v$VERSION\n"
        unless $pcclines[0] =~ /^#.*CVS.*Revision/i;
  ...
  if(keys %filelist_missing)
  {
    ...
    {
      mywarn "Can't find the file: $lstfile",
        $filelist_missing{$lstfile}[0], $filelist_missing{$lstfile}[1];
    }
  }
  ...
  if(keys %filelist_notpcc && !$cl_options{gamemode})
  {
    ...
    foreach my $file (sort keys %filelist_notpcc)
    {
...
{
  $files_to_parse{"STDIN"} = $opt_filetype;
}
...
if($conversion_enable{'SPELL:Add TYPE tags'})
{
  ...
  for (sort keys %class_files)
  {
    push @files_to_parse_sorted, $_;
    delete $temp_files_to_parse{$_};
  }
}
...
if($conversion_enable{'CLASSSPELL convertion to SPELL'})
{
  ...
  {
    for (sort keys %{$classspell_files{$filetype}})
    {
      push @files_to_parse_sorted, $_;
      delete $temp_files_to_parse{$_};
    }
...
if($conversion_enable{'CLASSSKILL convertion to CLASS'})
{
  ...
  for (sort keys %classskill_files)
  {
    push @files_to_parse_sorted, $_;
    delete $temp_files_to_parse{$_};
  }
...
foreach my $file (@files_to_parse_sorted)
{
  ...
  {
    # We read only what we know needs to be process
    next unless ref($validfiletype{$files_to_parse{$file}}) eq 'CODE';

    my $ok = open LSTFILE, "$file";

    if(!$ok)
    {
      mywarn("Cannot open:$!",$file);
      next
    }

    @lines = <LSTFILE>;

    close LSTFILE or die "Cannot close $file:$!";
  ...
  $cvs_line = ($cvs_line =~ /(\# cvs.*revision.*author.*?) -- /i)[0] || '# CVS $'.'Revision: $ $'.'Author: $';

  # Read the full file into the @lines array
  chomp(@lines);
  $numberofcf += s/[\x0d\x0a]//g for @lines;

  mywarn "$numberofcf extra CF found.", $file if $numberofcf;

  if(ref($validfiletype{$files_to_parse{$file}}) eq "CODE")
  {
#    $file_for_error = $file;
    my ($newlines_ref) = &{$validfiletype{$files_to_parse{$file}}}($files_to_parse{$file},
                                                                   \@lines,
                                                                   $file);

    # Let's remove the tralling white spaces
    s/\s+$// for (@$newlines_ref);

##    # We remove the " for the lines.
##    my $number_of_quote = 0;
##    @$newlines_ref = map { $number_of_quote += s/\"/\'/g unless /^\s*#/; $_ } @$newlines_ref;
##    mywarn "Found and removed $number_of_quote quotes in file.", $file if $number_of_quote;

    # We compare the result with the orginal file.
    # If there are no modification, we do not create the new files

    next if !$writefiletype{$files_to_parse{$file}};

    my $same = 0;
    if(!$numberofcf && $cvs_present && scalar(@lines) == scalar(@$newlines_ref))
    {
      $same = 1;
      for(my $index = 0; $index < scalar(@lines); $index++)
      {
        $same = 0, next if $lines[$index] ne $newlines_ref->[$index];
      }
    }
    ...
    if($opt_outputpath)
    {
      ...
      open NEWFILE, ">$newfile" or die "Cannot create $file: $!";
    ...
    print NEWFILE "$cvs_line -- $today -- reformated by $SCRIPTNAME v$VERSION\n"
      if $opt_outputpath || (*NEWFILE eq *STDOUT);

    # We print the result
    foreach my $line (@$newlines_ref)
    {
      next if $line eq '#$$$ CVS comment $$$';
      #$line =~ s/\s+$//;
      print NEWFILE "$line\n" if $opt_outputpath || (*NEWFILE eq *STDOUT);
    }

    close NEWFILE unless $file eq "STDIN" || !$opt_outputpath;
  }
  else
  {
...
if($opt_outputpath && scalar(@modified_files))
{
  $opt_outputpath =~ tr!/!\\! if $^O eq "MSWin32";
  set_mywarn_header "================================================================\n" .
...
if($conversion_enable{'Generate BONUS and PRExxx report'})
{
  ...
  my $first = 1;
  foreach my $linetype (sort keys %bonus_prexxx_tag_report)
  {
    print STDERR "\n" unless $first;
    $first = 0;
    print STDERR "Line Type: $linetype\n";

    foreach (sort keys %{$bonus_prexxx_tag_report{$linetype}})
    {
      print STDERR "  $_\n";
    }
  }
...
if($opt_report)
{
  ...
  my $first = 1;
  foreach my $linetype (sort keys %{$count_tags{"Valid"}})
  {
    next if $linetype eq "Total";

    print STDERR "\n" unless $first;
    print STDERR "Line Type: $linetype\n";

    foreach my $tag (sort report_tag_sort keys %{$count_tags{"Valid"}{$linetype}})
    {
      my $tagdisplay = $tag;
      $tagdisplay .= "*" if $master_mult{$linetype}{$tag};
      my $line = "     $tagdisplay";
      $line .= (" " x (26 - length($tagdisplay))) . $count_tags{"Valid"}{$linetype}{$tag};
      print STDERR "$line\n";
    }
  ...
  foreach my $tag (sort report_tag_sort keys %{$count_tags{"Valid"}{"Total"}})
  {
    my $line = "     $tag";
...
if(exists $count_tags{"Invalid"})
{
  ...
  my $first = 1;
  foreach my $linetype (sort keys %{$count_tags{"Invalid"}})
  {
    next if $linetype eq "Total";
    ...
    foreach my $tag (sort report_tag_sort keys %{$count_tags{"Invalid"}{$linetype}})
    {
  ...
  foreach my $tag (sort report_tag_sort keys %{$count_tags{"Invalid"}{"Total"}})
  {
    my $line = "     $tag";
...
if($opt_xcheck)
{
  ...
  for my $file (sort keys %to_report)
  {
    for my $line_ref (sort { $a->[0] <=> $b->[0] } @{$to_report{$file}})
    {
      ...
      # We add the ** to indicate that EQMOD Key are just a warning
      $message = '**' . $message if $line_ref->[1] eq 'EQUIPMOD Key';
      mywarn $message, $file, $line_ref->[0] ;
    }
  }
  ...
  for my $file (sort keys %to_report)
  {
    for my $line_ref (sort { $a->[0] <=> $b->[0] } @{$to_report{$file}})
    {
      mywarn qq(No $line_ref->[1] type found for "$line_ref->[2]"),
        $file, $line_ref->[0] ;
    }
  }
  ...
  if($cl_options{missing_header})
  {
    ...
    for my $linetype (sort keys %missing_headers)
    {
      ...
      foreach my $header (sort report_tag_sort keys %{$missing_headers{$linetype}})
      {
        print STDERR "     $header\n";
      }
...
if($conversion_enable{'Export lists'})
{
  close CLASS or die "Can't close class.csv:$!";
  close DOMAIN or die "Can't close domain.csv:$!";
  close EQUIPMENT or die "Can't close equipment.csv:$!";
  close EQUIPMOD or die "Can't close equipmod.csv:$!";
  close FEAT or die "Can't close feat.csv:$!";
  close LANGUAGE or die "Can't close language.csv:$!";
  close SKILL or die "Can't close skill.csv:$!";
  close SPELL or die "Can't close spell.csv:$!";
  close VARIABLE or die "Can't close variable.csv:$!" if $opt_xcheck;
  close PCC or die "Can't close pcc.csv:$!";
}
...
#             $file_for_error = File name to use with mywarn
...
sub FILETYPE_parse($\@$)
{
  ...
  for my $line (@$lines_ref)
  {
    ...
    {
      mywarn qq(!! Can\'t find the line type for "$new_line"),
    ...
    {
      mywarn qq(SUB line "$curent_linetype" is not preceded by a MAIN line),
        $file_for_error, $line_for_error
          if $last_main_line == -1;
    }
    ...
    my @tokens = grep {$_ ne ""} map {s/\s+$//;s/^\s+//;$_} split $sep, $new_line;

    #First, we deal with the tag-less columns
    foreach my $column (@{$column_with_no_tag{$curent_linetype}})
    {
      last unless @tokens;
...
#      $tokens[0] =~ s/^\s+//;
      ...
      mywarn qq(Removing quotes around the '$tokens[0]' tag), $file_for_error, $line_for_error
        if $tokens[0] =~ s/^"(.*)"$/$1/;
      ...
      {
        my $check_mod = $line_info->{RegExIsMod} || qr(^(.*)\.(MOD|FORGET|COPY=[^\t]+)$)i;

        if($line_info->{ValidateKeep})
        {
          if($curent_token =~ $check_mod)
          {
            # We keep track of the .MOD type tags to
            # later validate if they are valid
            push @{$referer{$curent_linetype}{$1}},
            ...
            if($opt_xcheck)
            {
              ...
              if($line_info->{RegExGetEntry})
              {
                if($entry =~ $line_info->{RegExGetEntry})
                {
                  ...
                  {
                    add_to_xcheck_tables($line_info->{IdentRefType},
                ...
                {
                  mywarn qq(!! Cannot find the $curent_linetype name),
                    $file_for_error, $line_for_error;
                }
              }
    ...
    for my $token (@tokens)
    {
      ...
      if($key)
      {
        ...
        {
          mywarn("The tag \"$key\" should not be used more than once on the same $curent_linetype line.\n",
                 $file_for_error, $line_for_error);
        }

        $line_tokens{$key} = exists $line_tokens{$key} ? [ @{$line_tokens{$key}}, $token ] : [ $token ];
      }
      else
      {
        mywarn "No tags in \"$token\"\n", $file_for_error, $line_for_error;
        $line_tokens{$token} = $token;
      }
    }

    my $newline = [ $curent_linetype,
                    \%line_tokens,
                    $last_main_line,
                    $curent_entity,
                    $line_info,
    ...
    additionnal_line_parsing %line_tokens, $curent_linetype, $file_for_error,
                             $line_for_error, $newline;
  ...
  return $lines_ref unless $opt_outputpath && $writefiletype{$file_type};

  # Now on to all the non header lines.
  for(my $line_index = 0; $line_index < @newlines; $line_index++)
  {
    # We skip the text lines and the header lines
    next if ref($newlines[$line_index]) ne 'ARRAY' ||
    ...
    if($mode == SINGLE || $format == LINE)
    {
      ...
      elsif($header == LINE_HEADER)
      {
        ...
        $col_length{$_} = mylength($line_tokens->{$_})
          for (keys %$line_tokens);
        ...
        for my $tag (@{$master_order{$curent_linetype}})
        {
          ...
          next unless exists $col_length{$tag};
    ...
    elsif($mode == MAIN)
    {
      if($format == BLOCK)
      {
        ...
        for(my $index = $line_index; $index < @newlines; $index++)
        {
          ...
          # if the a '###Block' comment is found,
          # we are out of the block
          last if (ref($newlines[$index]) eq 'ARRAY' &&
          ...
          next if ref($newlines[$index]) ne 'ARRAY' ||
...
#
# This function return a list of two elements. The first one is
# a return code, the second one is the effective TAG if any.
#
...
sub parse_tag($$$$)
{
  ...
  # We remove the inclosing quotes if any
  mywarn qq(Removing quotes around the '$tag_text' tag), $file_for_error, $line_for_error
    if $tag_text =~ s/^"(.*)"$/$1/;
  ...
  if($tag eq 'ADD')
  {
    ...
    else
    {
      ...
      {
        mywarn "Invalid ADD tag \"$tag_text\" found in $linetype.",
               $file_for_error, $line_for_error;
  ...
  if($tag eq 'BONUS')
  {
    ...
    # No valid bonus type was found
      ...
      mywarn "Invalid BONUS:$bonus_type tag \"$tag_text\" found in $linetype.",
             $file_for_error, $line_for_error;
      $no_more_error = 1;
    }
    else
    {
      ...
      mywarn "Invalid BONUS tag \"$tag_text\" found in $linetype",
        $file_for_error, $line_for_error;
      $no_more_error = 1;
    }
  }
  ...
  if($tag eq 'AUTO')
  {
    ...
    {
      $value =~ s/^$auto_type// and $found_auto_type = $auto_type and last;
    }
    ...
    else
    {
      ...
      if($value =~ /^([^=:|]+)/)
      {
        ...
        mywarn "Invalid $tag:$1 tag \"$tag_text\" found in $linetype.",
               $file_for_error, $line_for_error;
      }
      else
      {
        ...
        mywarn "Invalid AUTO tag \"$tag_text\" found in $linetype",
          $file_for_error, $line_for_error;
      }
      $no_more_error = 1;
  ...
  if($tag eq 'SPELLLEVEL')
  {
    ...
    else
    {
      ...
      if($value =~ /^([^=:|]+)/)
      {
        $count_tags{"Invalid"}{"Total"}{"$tag:$1"}++;
        $count_tags{"Invalid"}{$linetype}{"$tag:$1"}++;
        mywarn "Invalid SPELLLEVEL:$1 tag \"$tag_text\" found in $linetype.",
               $file_for_error, $line_for_error;
      }
      else
      {
        ...
        mywarn "Invalid SPELLLEVEL tag \"$tag_text\" found in $linetype",
          $file_for_error, $line_for_error;
      }
      $no_more_error = 1;
    }
  ...
  {
    mywarn("The tag \"$tag\" from \"$tag_text\" is not in the $linetype tag list\n",
           $file_for_error, $line_for_error);
  ...
  if(exists $tag_fix_value{$tag})
  {
    ...
    if    ($newvalue eq 'Y') { $newvalue = 'YES' }
    elsif ($newvalue eq 'N') { $newvalue = 'NO' }
    elsif ($newvalue eq 'W') { $newvalue = 'WEIGHT' }
    elsif ($newvalue eq 'Q') { $newvalue = 'QUALIFY' }
    elsif ($newvalue eq 'P') { $newvalue = 'PROFICIENT' };

    # Is this a proper value for the tag?
    if (!exists $tag_fix_value{$tag}{$newvalue})
    {
      mywarn qq(Invalid value "$value" for tag "$real_tag"),
        $file_for_error, $line_for_error;
    }
    # Was the tag remformated ?
    elsif ($value ne $newvalue)
    {
      mywarn qq(==> Replaced "$real_tag:$value" by "$real_tag:$newvalue"),
        $file_for_error, $line_for_error;
      $value = $newvalue;
    }
  }
  ...
  my $need_sep = index($real_tag,':') == -1 ? ':' : '';

  # We change the tag_text value from the caller
  # This is very ugly but it gets th job done
  $_[0] = $real_tag;
  $_[0] .= $need_sep . $value if defined $value && $value ne 'MULTIPREREQS';
...
# ------------
#
# This function store data for later validation.
#
...
my %valid_NATURALATTACKS_type = (
      # WEAPONTYPE defined in gameMode
      Bludgeoning         =>1,
      Piercing            =>1,
      Slashing            =>1,
      Fire                =>1,
      ...
      # WEAPONCATEGORY defined in gameMode
      Simple              =>1,
      Martial             =>1,
      Exotic              =>1,
      Natural             =>1,
...
sub validate_tag($$$$$)
{
  ...
  unless(defined $tag_value)
  {
    mywarn qq(!! The tag "$tag_name" is missing a value (or you forgot a : somewhere)),
      $file_for_error,$line_for_error;
    return;
  }
  ...
  {
    mywarn qq(* HITDICESIZE is deprecated, use HITDIE instead.),
      $file_for_error,$line_for_error
  }
  if($tag_name eq 'SPELL' && $linetype ne 'PCC')
  {
    mywarn qq(* SPELL is deprecated, use SPELLS instead.),
      $file_for_error,$line_for_error
  }
  ...
  elsif($tag_name =~ /^!?PRE/)
  {
    ...
    return validate_pre_tag($tag_name,$tag_value,"",$linetype,$file_for_error,$line_for_error);
  }
  elsif(index($tag_name,'BONUS') == 0)
  {
    ...
    if($tag_value =~ /(!?PRE[A-Z]*):([^|]*)/)
    {
      ...
      validate_pre_tag($1,$2,"$tag_name$tag_value",$linetype,$file_for_error,$line_for_error);
    }

    if($tag_name eq 'BONUS:FEAT')
    {
      # BONUS:FEAT|POOL|<formula>|<prereq list>|<bonus type>

      my @list_of_param = split '\|', $tag_value;

      unless((shift @list_of_param) eq 'POOL')
      {
        # For now, only POOL is valid here
        mywarn qq(Only POOL is valid as second paramater for BONUS:FEAT: "$tag_name$tag_value"),
            $file_for_error, $line_for_error;
      }

      # The next parameter is the formula
      push @xcheck_to_process, ['DEFINE Variable',qq(@@" in "$tag_name$tag_value),
                                $file_for_error, $line_for_error,
                                extract_var_name((shift @list_of_param),
                                                 "$tag_name$tag_value",
                                                 $file_for_error,
                                                 $line_for_error)
                               ];
      ...
      for my $param (@list_of_param)
      {
        ...
        {
          mywarn qq(Invalid parameter "$param" found in "$tag_name$tag_value"),
            $file_for_error, $line_for_error;
        }
      }
      ...
      if($type_present > 1)
      {
          mywarn qq(There should be only one "TYPE=" in "$tag_name$tag_value"),
            $file_for_error, $line_for_error;
      }
    }
    if($tag_name eq 'BONUS:MOVEADD' || $tag_name eq 'BONUS:MOVEMULT' || $tag_name eq 'BONUS:POSTMOVEADD')
    {
      ...
      for my $type (split ',', $type_list)
      {
        if($type =~ /^TYPE(=|\.)(.*)/)
        {
          push @xcheck_to_process, ['MOVE Type',qq(TYPE$1@@" in "$tag_name$tag_value),
                                    $file_for_error, $line_for_error,
                                    $2
                                   ];
        }
        ...
        {
          mywarn qq(Missing "TYPE=" for "$type" in "$tag_name$tag_value"),
            $file_for_error, $line_for_error;
        }
      }
    ...
    elsif($tag_name eq 'BONUS:SLOTS')
    {
      ...
      for my $type (split ',', $type_list)
      {
        ...
        {
          mywarn qq(Invalid slot type "$type" in "$tag_name$tag_value"),
            $file_for_error, $line_for_error;
        }
      }
    ...
    elsif($tag_name eq 'BONUS:VAR')
    {
      ...
      my ($var_name_list,@formulas) = (split '\|',$tag_value)[1,2];
  ...
  ##        $valid_entities{'DEFINE Variable'}{$var_name}++;
          push @formulas, $var_name;
        }
        else
        {
          mywarn qq(Invalid variable name "$var_name" in "$tag_name$tag_value"),
            $file_for_error, $line_for_error;
        }
      }

      # Second we deal with the formula
      # LIST is filtered out here as it may not be valid for the
  ...
  elsif($tag_name eq 'CLASSES' || $tag_name eq 'DOMAINS')
  {
    if($linetype eq 'SPELL')
    {
      ...
      for my $level (split '\|', $tag_value)
      {
        if($level =~ /(.*)=(\d+)/)
        {
          for my $entity (split ',', $1)
          {
            ...
            if(($tag_to_check eq "CLASS" &&
                 ($entity ne "ALL" && $entity ne "Arcane" && $entity ne "Divine")) ||
               $tag_to_check eq "DOMAIN")
            {
              push @xcheck_to_process, [$tag_to_check,$tag_name,
                                        $file_for_error, $line_for_error,
                                        $entity];

              mywarn qq("$entity" found more then once in $tag_name),
                $file_for_error, $line_for_error
                  if $seen{$entity}++;
            }
          }
        ...
        {
          mywarn qq(!! Missing "=level" after "$tag_name:$level"),
            $file_for_error,$line_for_error;
        }
      }
    }
    elsif($linetype eq 'SKILL')
    {
      ...
      for my $class (split '\|', $tag_value)
      {
        ...
        push @xcheck_to_process, ['CLASS',$tag_name,
                                  $file_for_error, $line_for_error,
                                  $class];
      }
    }
    elsif($linetype eq 'DEITY')
    {
      ...
      {
        # ALL is valid here
        next if $domain eq 'ALL';

        push @xcheck_to_process, ['DOMAIN',$tag_name,
                                  $file_for_error, $line_for_error,
                                  $domain];
      }
    }
  ...
  elsif($tag_name eq 'ADD:SPELLCASTER')
  {
    ...
    if($tag_value =~ /\((.*)\)(.*)/)
    {
      ...
      # ANY, ARCANA and DIVINE are spcial hardcoded cases for
      # the ADD:SPELLCASTER tag.
      push @xcheck_to_process, ['CLASS',qq(@@" in "$tag_name$tag_value),
                                $file_for_error, $line_for_error,
                                grep { uc($_) ne 'ANY'    &&
                                       uc($_) ne 'ARCANE' &&
                                       uc($_) ne 'DIVINE'
      ...
      # Second, we deal with the formula
      push @xcheck_to_process, ['DEFINE Variable',qq(@@" from "$formula" in "$tag_name$tag_value),
                                $file_for_error, $line_for_error,
                                extract_var_name($formula,
                                                 "$tag_name$tag_value",
                                                 $file_for_error,
    ...
    {
      mywarn qq(Invalid syntax: "$tag_name$tag_value"),
        $file_for_error, $line_for_error;
    }
  }
    ...
    if($tag_name eq 'EQMOD')
    {
      ...
      for $_ (@key_list)
      {
        ...
        if($key)
        {
          ...
          push @xcheck_to_process, ['EQUIPMOD Key',qq(@@" in "$tag_name:$tag_value),
                                    $file_for_error, $line_for_error,
                                    $key];
        }
        else
        {
          mywarn qq(!! Cannot find the key for "$_" in "$tag_name:$tag_value"),
            $file_for_error, $line_for_error;
        }
      }
    }
  ...
  elsif($linetype ne 'PCC' &&
        ($tag_name eq 'ADD:FEAT' || $tag_name eq 'AUTO:FEAT' ||
         $tag_name eq 'FEAT' || $tag_name eq 'FEATAUTO' ||
         $tag_name eq 'VFEAT' || $tag_name eq 'MFEAT'))
  {
    my @feats;
    my $parent = NO;
...
#        s/&coma;/,/g for @feats;
      ...
      {
        mywarn qq(Invilid systax: "$tag_name$tag_value"),
          $file_for_error, $line_for_error
            if $tag_value;
      }
    }
...
#      s/&coma;/,/g for @feats;
    ...
    for my $feat (@feats)
    {
      ...
      {
        $feat = $1;
        validate_pre_tag($2,$3,"$tag_name:$tag_value",$linetype,$file_for_error,$line_for_error);
      }

      # We skip the PRExxx tags if it is a VFEAT tag.
    ...
    # To be processed later
    push @xcheck_to_process, ['FEAT',$message_format, $file_for_error, $line_for_error, @feats];
  }
  elsif($tag_name eq 'LANGAUTOxxx' || $tag_name eq 'LANGBONUS')
  {
    ...
    # used.
    push @xcheck_to_process, ['LANGUAGE',$tag_name, $file_for_error, $line_for_error,
  ...
  elsif($tag_name eq 'ADD:Language')
  {
    ...
    if($tag_value =~ /\((.*)\)/)
    {
      push @xcheck_to_process, ['LANGUAGE','ADD:Language(@@)', $file_for_error, $line_for_error,
                                 split ',',$1];
    }
    else
    {
      mywarn qq(Invalid syntax for "$tag_name$tag_value"),
        $file_for_error, $line_for_error;
    }
  }
  elsif($tag_name eq 'MOVE')
  {
    ...
    while(@list)
    {
      ...
      if($type =~ /^\d+$/)
      {
        mywarn qq(I was expecting a move type where I found "$type" in "$tag_name:$tag_value"),
          $file_for_error, $line_for_error;
        last;
      }
      else
      ...
      unless($value =~ /^\d+$/)
      {
        mywarn qq(I was expecting a number after "$type" and found "$value" in "$tag_name:$tag_value"),
          $file_for_error, $line_for_error;
        last;
      }
    }
  ...
  {
    # To be processed later
    push @xcheck_to_process, ['RACE',$tag_name, $file_for_error, $line_for_error,
                               split '\|',$tag_value];
  }
  elsif($tag_name eq 'CSKILL' || $tag_name eq 'CCSKILL' ||
        $tag_name eq 'MONCSKILL' || $tag_name eq 'MONCCSKILL')
  {
    # To be processed later
    push @xcheck_to_process, ['SKILL',$tag_name, $file_for_error, $line_for_error,
                               split '\|',$tag_value];
  }
  ...
  elsif($tag_name eq 'SPELLS')
  {
    ...
    for my $param (@list_of_param)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          mywarn qq(** the DC value is missing for "$param" in "$tag_name:$tag_value"),
            $file_for_error, $line_for_error;
        }
      }
    }

    push @xcheck_to_process, ['SPELL',$tag_name,
                              $file_for_error, $line_for_error,
                              @spells];
    ...
    if($nb_times != 1)
    {
      if($nb_times)
      {
        mywarn qq(TIMES= should not be used more then once in "$tag_name:$tag_value"),
          $file_for_error, $line_for_error;
      }
      else
      {
        mywarn qq(** the TIMES= parameter is missing in "$tag_name:$tag_value"),
          $file_for_error, $line_for_error;
      }
    }
    ...
    if($nb_casterlevel != 1)
    {
      if($nb_casterlevel)
      {
        mywarn qq(CASTERLEVEL= should not be used more then once in "$tag_name:$tag_value"),
          $file_for_error, $line_for_error;
      }
      else
      {
        mywarn qq(** the CASTERLEVEL= parameter is missing in "$tag_name:$tag_value"),
          $file_for_error, $line_for_error;
      }
    }
  ...
  elsif(index($tag_name,'SPELLLEVEL:') == 0)
  {
    ...
    if($tag_name eq "SPELLLEVEL:CLASS")
    {
      ...
      if(my $working_value = $tag_value)
      {
        while($working_value)
        {
          if($working_value =~ s/\|([^|]+)\|([^|]+)//)
          {
            ...
            {
              mywarn qq(Invalid syntax for "$class" in "$tag_name$tag_value"),
                $file_for_error, $line_for_error;
            }

            # The SPELL names
            # To be processed later
            push @xcheck_to_process, ['SPELL',qq(@@" in "$tag_name$tag_value),
                                      $file_for_error, $line_for_error,
                                      split ',', $spells];
          }
          else
          {
            mywarn qq(Invalid class/spell list paring in "$tag_name$tag_value"),
              $file_for_error, $line_for_error;
            $working_value = "";
          }
        }
      ...
      {
        mywarn qq(No value found for "$tag_name"),
          $file_for_error, $line_for_error;
      }
    }
      ...
      if(my $working_value = $tag_value)
      {
        while($working_value)
        {
          if($working_value =~ s/\|([^|]+)\|([^|]+)//)
          {
            ...
            if($domain =~ /([^=]+)\=(\d+)/)
            {
              push @xcheck_to_process, ['DOMAIN',qq(@@" in "$tag_name$tag_value),
                                        $file_for_error, $line_for_error, $1];
            }
            else
            {
              mywarn qq(Invalid syntax for "$domain" in "$tag_name$tag_value"),
                $file_for_error, $line_for_error;
            }
          ...
          {
            mywarn qq(Invalid domain/spell list paring in "$tag_name$tag_value"),
              $file_for_error, $line_for_error;
            $working_value = "";
          }
        }
  ...
  elsif($tag_name eq 'DEFINE')
  {
    ...
    if($var_name)
    {
      if($var_name =~ /^[a-z][a-z0-9_]*$/i)
      {
        ...
        if($conversion_enable{'Export lists'})
        {
          ...
          $file =~ tr!/!\\!;
          print VARIABLE qq("$var_name","$line_for_error","$file"\n);
        }
    ...
    {
      mywarn qq(* I was not able to find a proper variable name in "$tag_name:$tag_value"),
        $file_for_error, $line_for_error;
    }
        ...
        ($tag_name eq 'TARGETAREA' || $tag_name eq 'DURATION' || $tag_name eq 'DESC'))
  {
    ...
    {
      $result = "";
      ($result,$value) = ($value =~ /\(([^()]*CASTERLEVEL[^()]*)\)(.*)/);
      if($result)
      {
        push @xcheck_to_process, ['DEFINE Variable',qq(@@" in "$tag_name:$tag_value),
                                  $file_for_error, $line_for_error,
                                  extract_var_name($result,
                                                   "$tag_name:$tag_value",
                                                   $file_for_error,
                                                   $line_for_error)
                                 ];
  ...
  elsif($tag_name eq 'NATURALATTACKS')
  {
    ...
    for my $entry (split '\|', $tag_value)
    {
      ...
      if(+@parameters == 4)
      {
        ...
        mywarn qq(3rd parameter should be a number in "NATURALATTACKS:$entry"),
          $file_for_error, $line_for_error
            unless $parameters[2] =~ /^\*?\d+$/;

        # Are the types valid EQUIPMENT types?
        push @xcheck_to_process, ['EQUIPMENT TYPE',qq(@@" in "$tag_name:$entry),
                                  $file_for_error, $line_for_error,
      ...
      {
        mywarn qq(Wrong number of parameter for "NATURALATTACKS:$entry"),
          $file_for_error, $line_for_error;
      }
    }
  }
...
##        mywarn "** Typeless CHOOSE found: \"$tag_name:$tag_value\" in $linetype.",
##               $file_for_error, $line_for_error;
##      }
##      else
##      {
...
##        mywarn "Invalid CHOOSE:$choose_type tag \"$tag_name:$tag_value\" found in $linetype.",
##               $file_for_error, $line_for_error;
##      }
##    }
##    elsif(!$choose_type)
...
##      mywarn "Invalid CHOOSE tag \"$tag_name:$tag_value\" found in $linetype",
##        $file_for_error, $line_for_error;
##    }
##  }
...
# Paramter: $tag_name         Name of the tag (before the :)
#           $tag_value        Value of the tag (after the :)
#           $enclosing_tag    When the PRExxx tag is used in
#                             another tag
#           $linetype         Type for the courent file
#           $file_for_error   Name of the courent file
#           $line_for_error   Number of the courent line
...
sub validate_pre_tag($$$$$$)
{
  my ($tag_name, $tag_value, $enclosing_tag,
      $linetype, $file_for_error, $line_for_error) = @_;
  ...
  if (!length($tag_value) && $tag_name ne "PRE:.CLEAR")
  {
    ...
    mywarn $message, $file_for_error, $line_for_error;

    return;
  }


#warn "DEBUG:$tag_name; $tag_value; $enclosing_tag;$linetype; $file_for_error; $line_for_error\n";
  ...
  if($pretag eq 'PRECLASS' || $pretag eq 'PRECLASSLEVELMAX')
  {
    ...
    else
    {
      ...
      my $message = qq(*Deprecated syntax: "$tag_name:$tag_value");
      $message .= qq( found in "$enclosing_tag") if $enclosing_tag;
      mywarn $message, $file_for_error, $line_for_error;
    }
  ...
  elsif($pretag eq 'PREDEITYDOMAIN' || $pretag eq 'PREDOMAIN')
  {
    ...
    push @xcheck_to_process, ['DOMAIN',$tag_name, $file_for_error, $line_for_error, @domains];
  }
  ...
  elsif($pretag eq 'PREITEM')
  {
    ...
    push @xcheck_to_process, ['EQUIPMENT',$tag_name, $file_for_error, $line_for_error, @items];
  }
  ...
  elsif($pretag eq 'PREMOVE')
  {
    ...
    for my $move (@moves)
    {
      ...
      if($move =~ /^([^=]*)=([^=]*)$/)
      {
        push @xcheck_to_process, ['MOVE Type',$tag_name, $file_for_error, $line_for_error,
                                  $1];
        ...
        unless($value =~ /^\d+$/)
        {
          my $message = qq(Not a number after the = for "$move" in "$tag_name:$tag_value");
          $message .= qq( found in "$enclosing_tag") if $enclosing_tag;
          mywarn $message, $file_for_error, $line_for_error;
        }
      }
  ...
  elsif($pretag eq 'PREMULT')
  {
    ...
    while($inside = Text::Balanced::extract_bracketed($working_value, '[]', qr([^[]*)))
    {
      ...
      if($tag)
      {
        validate_pre_tag($tag, $value, $emb_tag_name,
                         $linetype, $file_for_error, $line_for_error);
      }
      else
      {
        # No PRExxx tag found inside the PREMULT
        mywarn qq(!No valid PRExxx tag found in "$inside" inside "PREMULT:$tag_value"),
          $file_for_error, $line_for_error
      }
    }
  }
  elsif($pretag eq 'PRERACE')
  {
    ...
    my @races = split ',', $tag_value;

    if($races[0] =~ /\d+/)
    {
      shift @races;   # We drop the number at the beginning
    }
    else
    {
      # The PREtag doesn't begin by a number
      ...
      #$message .= qq( found in "$enclosing_tag") if $enclosing_tag;
      #mywarn $message, $file_for_error, $line_for_error;
      # Commented out for now, I don't think PRERACE has been converted.
    }
  ...
  elsif($pretag eq 'PRESPELL')
  {
    ...
    push @xcheck_to_process, ['SPELL',"$tag_name:@@", $file_for_error, $line_for_error, @spells];
  }
  elsif($pretag eq 'PREVAR')
  {
    my (@formulas) = split ',',$tag_value;
  ...
  {
    mywarn qq(Unknown PRExxx tag "$tag_name" found in "$enclosing_tag"),
      $file_for_error, $line_for_error;
  }
}
...
my %Hardcoded_Variables = (
     ...
     DEX            => 1,
     DEXSCORE       => 1,
     DVR            => 1,     # For the CMP stuff
     DVRSCORE       => 1,     # For the CMP stuff
     ENCUMBERANCE   => 1,
     GRAPPLESIZEMOD => 1,
     HD             => 1,
     INT            => 1,
...
#     LIST           => 1,
     ...
     NATIVELANGUAGES        => 1,
     PSIONLEVEL             => 1,
     VEHICLECRUISINGMPH     => 1,
     VEHICLEDEFENSE         => 1,
     VEHICLEHANDLING        => 1,
     VEHICLEHARDNESS        => 1,
     VEHICLESPEED           => 1,
     VEHICLETOPMPH          => 1,
     VEHICLEWOUNDPOINTS     => 1,

   );

sub add_to_xcheck_tables(@)
{
  my ($entry_type,$tag_name,$file_for_error,$line_for_error,@list) = (@_,"");

  # We removed the empty elements in the list
  #my $nb_list = +@list;
  #warn qq(DEBUG:$entry_type,$tag_name,$file_for_error,$line_for_error,@list);
  #dump(@_);
  @list = grep {$_ ne ""} @list;
  ...
  elsif($entry_type eq 'FEAT')
  {
    for my $feat (@list)
    {
      ...
      next if $feat eq 'CHECKMULT' && $tag_name =~ /PREFEAT/;

      # We ignore LIST if used within an ADD:FEAT tag
      next if $feat eq 'LIST' && $tag_name eq 'ADD:FEAT';
      ...
      if($feat =~ /(.*?[^ ]) ?\((.*)\)/)
      {
        ...
        if($valid_sub_entities{'FEAT'}{$1})
        {
          ...
          while($valid_sub_entities{'FEAT'}{$feat_to_check} =~ /^FEAT=(.*)/)
          {
            ...
            {
              mywarn qq(Cannot find the sub-entity for "$original_feat"),
                $file_for_error, $line_for_error;
              $feat_to_check = "";
              last;
            }
...
#             $file_for_error : Filename to use in mywarn
#             $line_for_error : Line number to use in mywarn

#open FORMULA, ">formula.txt" or die "Can't open formula: $!";
...
#  print FORMULA "$formula\n" unless $formula =~ /^[0-9]+$/;
  ...
  while($formula =~ s/([a-z][a-z0-9_]*=[a-z0-9_ ='{}]*)//i)
  {
    ...
    if(@values > 2)
    {
      ...
      mywarn qq(Too many = in "$1" found in "$tag"),
        $file_for_error, $line_for_error;
    }
    elsif($values[0] eq 'BL' ||               # [ 1104117 ] BL is a valid variable, like CL
          $values[0] eq 'CL' ||
          $values[0] eq 'CLASS' ||
          $values[0] eq 'CLASSLEVEL')
    {
      # Convert {} to () for proper validation
      $values[1] =~ tr/{}/()/;
      push @xcheck_to_process, ['CLASS',qq(@@" in "$tag),
                                $file_for_error, $line_for_error,
                                $values[1]
                               ];
    }
          ...
          $values[0] eq 'SKILLTOTAL')
    {
      # Convert {} to () for proper validation
      $values[1] =~ tr/{}/()/;
      push @xcheck_to_process, ['SKILL',qq(@@" in "$tag),
                                $file_for_error, $line_for_error,
                                $values[1]
                               ];
    }
    ...
    {
      mywarn qq(Invalid variable "$values[0]" before the = in "$1" found in "$tag"),
        $file_for_error, $line_for_error;
    }
  }
  ...
  for my $var_name ($formula =~ /([a-z%][a-z0-9_]*)/gi)
  {
    ...
    next if(index($var_name,'MAX') != -1 || index($var_name,'MIN') != -1 ||
...
sub additionnal_tag_parsing($$$$$)
{
     ...
     $linetype eq 'TEMPLATE')
  {
    ...
    $_[0] = 'HITDIE';
    mywarn qq(=> Changing "$tag_name:$tag_value" to "$_[0]:$_[1]"),
      $file_for_error,$line_for_error;
  ...
  if($conversion_enable{'ALL:CMP remove PREALIGN'})
  {
    if($tag_value =~ /PREALIGN/)
    {
      ...
      {
        mywarn qq(!! PREALIGN found in PREMULT, you will have to remove it yourself "$tag_name$ponc$tag_value"),
          $file_for_error,$line_for_error;
      }
      elsif($tag_name =~ /^BONUS/ || $tag_name eq 'SA' || $tag_name eq 'VFEAT')
      {
        $_[1] = join('|',grep {!/^(!?)PREALIGN/} split '\|', $tag_value);
        mywarn qq(=> Replacing "$tag_name$ponc$tag_value" with "$_[0]$ponc$_[1]"),
          $file_for_error,$line_for_error;
      }
      else
      {
        mywarn qq(!! Found PREALIGN were I wasn't expecting it "$tag_name$ponc$tag_value"),
          $file_for_error,$line_for_error;
      }
    }
  }
...
#        mywarn qq(=> Replacing "$old_key" by "$Key_conversion_56{$old_key}" in "$tag_name:$tag_value"),
#          $file_for_error,$line_for_error;
#      }
#    }
#  }
  ...
  if($conversion_enable{'RACE:CSKILL to MONCSKILL'} &&
     ...
     $tag_name eq "CSKILL")
  {
    mywarn qq(=> Found CSKILL in RACE file),
      $file_for_error,$line_for_error;
     ...
     $tag_value =~ /DnD_/)
  {
    ...
    map {$has_3e  = 1 if $_ eq "3e";  $has_DnD_v30e = 1 if $_ eq "DnD_v30e";
         $has_35e = 1 if $_ eq "35e"; $has_DnD_v35e = 1 if $_ eq "DnD_v35e";} split '\|', $tag_value;
     ...
     $tag_value =~ /^\|(BAB)\|/i)
  {
    ...
    if($1 ne 'BAB')
    {
      mywarn qq(=> Changing "$tag_name$tag_value" to "$_[0]$_[1]" (BAB must be in uppercase)),
        $file_for_error,$line_for_error;
       ...
       (($linetype eq 'RACE' || $linetype eq 'TEMPLATE') && $is_predefaultmonster))
    {
      if(!$is_type)
      {
        ...
        mywarn qq(=> Adding "|TYPE=Base.REPLACE" to "$tag_name$tag_value"),
          $file_for_error,$line_for_error;
      }
      else
      {
        ...
        if(!$is_type_replace && $is_type_base)
        {
          ...
          mywarn qq(=> Adding ".REPLACE" to "$tag_name$tag_value"),
            $file_for_error,$line_for_error;
        }
        elsif(!$is_type_base)
        {
          mywarn qq(** Verify the TYPE of "$tag_name$tag_value"),
            $file_for_error,$line_for_error;
        }
      }
    }
    else
    {
      ...
      # inspection.
      mywarn qq(** Verify this tag "$tag_name$tag_value"),
        $file_for_error,$line_for_error;
    }
  }
     ...
     $tag_name eq "DEFINE")
  {
    if($tag_value =~ /COUNT\[FEATTYPE=/i)
    {
      ...
      {
        $_[1] = $new_value;
        mywarn qq(=> Replacing "$tag_name:$tag_value" by "$_[0]:$_[1]"),
          $file_for_error, $line_for_error;
      }
    }
  }
...
sub validate_line(\%$$$)
{
  ...
  unless($linetype eq 'SOURCE' ||
         $linetype eq 'KIT LANGAUTO' ||
         $file_for_error =~ m'\.PCC$'i ||
         $linetype eq 'COMPANIONMOD')     # FOLLOWER:Class1,Class2=level
  {
    ...
    mywarn qq("," (coma) should not be used in line identifier name: $identifier),
      $file_for_error,$line_for_error
        if $identifier =~ /,/;
  }
  ...
  if(0 && $linetype eq 'SPELL') # disabled for now.
  {
    ...
    {
      mywarn qq(** No CLASSES or DOMAINS tag found for SPELL "$line_ref->{'000SpellName'}[0]"),
        $file_for_error,$line_for_error
  ...
  elsif($linetype eq "FEAT")
  {
    ...
    {
      mywarn qq(** The CHOOSE tag is mandantory when MULT:YES is present in FEAT "$line_ref->{'000FeatName'}[0]"),
        $file_for_error,$line_for_error;
    }
    elsif($hasCHOOSE && !$hasMULT && $line_ref->{'CHOOSE'}[0] !~ /CHOOSE:SPELLLEVEL/i)
    {
      # The CHOOSE:SPELLLEVEL is exampted from this particular rule.
      mywarn qq(** The MULT:YES tag is mandantory when CHOOSE is present in FEAT "$line_ref->{'000FeatName'}[0]"),
        $file_for_error,$line_for_error;
    }

    if($hasSTACK && !$hasMULT)
    {
      mywarn qq(** The MULT:YES tag is mandantory when STACK:YES is present in FEAT "$line_ref->{'000FeatName'}[0]"),
        $file_for_error,$line_for_error;
    }
    ...
    if($hasCHOOSE)
    {
      ...
      {
        $valid_sub_entities{'FEAT'}{$feat_name} = 'WEAPONPROFS';
      }
      ...
      elsif($choose =~ /^CHOOSE:(?:COUNT=\d+\|)?(.*)/)
      {
        ...
        # It adds to the valid entities intead of the
        # valid sub-entities.
  ...
  elsif($linetype eq "EQUIPMOD")
  {
    ...
    if(exists $line_ref->{'KEY'})
    {
      ...
      {
        mywarn qq(!! Could not parse the KEY in "$line_ref->{'KEY'}[0]"),
          $file_for_error,$line_for_error;
      }
    }
    else
    {
      mywarn qq(** No KEY tag found for "$line_ref->{$column_with_no_tag{'EQUIPMOD'}[0]}[0]"),
        $file_for_error,$line_for_error;
    }
  }
  elsif($linetype eq "CLASS")
  {
    ...
    {
      mywarn qq(Missing BONUS:CASTERLEVEL for "$line_ref->{$column_with_no_tag{'CLASS'}[0]}[0]"),
        $file_for_error,$line_for_error;
...
sub additionnal_line_parsing(\%$$$;$)
{
     ...
     && exists $line_ref->{'SPELL'})
  {
    ...
    for my $tag (@{$line_ref->{'SPELL'}})
    {
      ...
      while(@elements)
      {
        if(+@elements < 3)
        {
          mywarn qq(!! Wrong number of elements for "$tag_name:$tag_value"),
            $file_for_error,$line_for_error;
        }
      ...
      mywarn qq(=> Removing "$tag_name:$tag_value"),
        $file_for_error,$line_for_error;
    }
  ...
  if($conversion_enable{'ALL:CMP remove PREALIGN'})
  {
    if(exists $line_ref->{'PREALIGN'})
    {
      ...
      mywarn qq(=> Removing $number PREALIGN tags),
       $file_for_error,$line_for_error
    }
    ...
    if(@{$line_ref->{'NATURALATTACKS'}} == 1)
    {
      ...
      if(@NatAttacks == 1)
      {
        ...
        if($NbAttacks eq '*1' && $Damage)
        {
          ...
          if($IsMelee && !$IsRanged)
          {
            ...
            if(exists $line_ref->{'BONUS:WEAPONPROF'})
            {
              ...
              unless($AlreadyThere)
              {
                ...
                mywarn qq(=> Added "$line_ref->{'BONUS:WEAPONPROF'}[0]" to go with "$line_ref->{'NATURALATTACKS'}[0]"),
                  $file_for_error,$line_for_error;
              }
            }
            else
          ...
          {
            mywarn qq(!! This natural attack is both Melee and Ranged "$line_ref->{'NATURALATTACKS'}[0]"),
              $file_for_error,$line_for_error;
          }
        }
      }
     ...
     exists $line_ref->{'MOVE'})
  {
    ...
    if($swim || $climb)
    {
      ...
      if(exists $line_ref->{'BONUS:SKILL'})
      {
        for my $skill (@{$line_ref->{'BONUS:SKILL'}})
        {
          if($skill =~ /^BONUS:SKILL\|([^|]*)\|(\d+)\|TYPE=Racial/i)
          {
            ...
            {
              $skill_list = join(',', sort(slipt (',',$skill_list),'Swim'));
              $skill = "BONUS:SKILL|$skill_list|8|TYPE=Racial";
              mywarn qq(=> Added Swim to "$skill"),
                $file_for_error,$line_for_error;
            }

            if($need_climb && $skill_rank == 8)
            {
              $skill_list = join(',', sort(slipt (',',$skill_list),'Climb'));
              $skill = "BONUS:SKILL|$skill_list|8|TYPE=Racial";
              mywarn qq(=> Added Climb to "$skill"),
                $file_for_error,$line_for_error;
            }

            if(($need_climb || $need_swim) && $skill_rank != 8)
            {
              mywarn qq(** You\'ll have to deal with this one yourself "$skill"),
                $file_for_error,$line_for_error;
            }
          }
        }
      ...
      if(exists $line_ref->{'BONUS:SKILLRANK'})
      {
          ...
          if($skillrank =~ /^BONUS:SKILLRANK\|(.*)\|(\d+)\|PREDEFAULTMONSTER:Y/)
          {
            ...
            if($climb && $skill_list =~ /climb/i)
            {
              if($skill_list eq "Climb")
              {
                ...
                if($skill_rank)
                {
                  $skillrank = "BONUS:SKILLRANK|Climb|$skill_rank|PREDEFAULTMONSTER:Y";
                  mywarn qq(=> Lowering skill rank in "$skillrank"),
                    $file_for_error,$line_for_error;
                }
                else
                {
                  mywarn qq(=> Removing "$skillrank"),
                    $file_for_error,$line_for_error;
              ...
              {
                mywarn qq(** You\'ll have to deal with this one yourself "$skillrank"),
                  $file_for_error,$line_for_error;
              }
            }
            ...
            if($swim && $skill_list =~ /swim/i)
            {
              if($skill_list eq "Swim")
              {
                ...
                if($skill_rank)
                {
                  $skillrank = "BONUS:SKILLRANK|Swim|$skill_rank|PREDEFAULTMONSTER:Y";
                  mywarn qq(=> Lowering skill rank in "$skillrank"),
                    $file_for_error,$line_for_error;
                }
                else
        ...
        delete $line_ref->{'BONUS:SKILLRANK'} unless @{$line_ref->{'BONUS:SKILLRANK'}};
      }
    }
  }
  ...
  {
    mywarn qq(=> Removing the SIZE tag in line "$line_ref->{$master_order{'WEAPONPROF'}[0]}[0]"),
      $file_for_error,$line_for_error;
     ...
     $filetype eq "RACE")
  {
    ...
    if(exists $line_ref->{'HANDS'})
    {
      ...
      {
        mywarn qq(** Invalid value in tag "$line_ref->{'HANDS'}[0]"),
          $file_for_error,$line_for_error;
        $needNoProfReq = 0;
      }
    }
    ...
    if($needNoProfReq && $nbHands)
    {
      if(exists $line_ref->{'AUTO:WEAPONPROF'})
      {
        mywarn qq(=> Adding "TYPE=NoProfReq" to tag "$line_ref->{'AUTO:WEAPONPROF'}[0]"),
          $file_for_error,$line_for_error;
      ...
      else
      {
        ...
        mywarn qq(=> Creating new tag "AUTO:WEAPONPROF|TYPE=NoProfReq"),
          $file_for_error,$line_for_error;
      }
    }
  }
  ...
  {
    mywarn qq(=> Change CSKILL for MONSKILL in "$line_ref->{'CSKILL'}[0]"),
      $file_for_error,$line_for_error;
  ...
  {
    mywarn qq(=> Removing "$line_ref->{'VISION'}[0]"),
      $file_for_error,$line_for_error;
    ...
    for my $vision_bonus (split ',',$2)
    {
      if($vision_bonus =~ /(\w+)\s*\((\d+)\'\)/)
      {
        ...
        mywarn qq(=> Adding "BONUS:VISION|$type|$bonus"),
          $file_for_error,$line_for_error;
      ...
      {
        mywarn qq(!!!! Do not know how to convert "VISION:.ADD,$vision_bonus"),
          $file_for_error,$line_for_error;
      }
    }

    mywarn qq(=> Adding "$newvision"), $file_for_error,$line_for_error;
     ...
     !exists $line_ref->{'SLOTS'})
  {
    ...
    if(exists $line_ref->{'TYPE'})
    {
      ...
      if($type =~ /(Boot|Glove|Bracer)/)
      {
           ...
           $1 eq 'Bracer' && $equipment_name =~ /bracers|bracelets/i)
        {
          ...
          mywarn qq(=> "SLOTS:2" added to "$equipment_name"),
            $file_for_error,$line_for_error;
        }
        else
        {
          mywarn qq("$equipment_name" is a $1),
            $file_for_error,$line_for_error;
        }
      }
    }
    ...
    {
      mywarn qq(!!! $equipment_name has no TYPE.),
        $file_for_error,$line_for_error
          ...
          (!exists $line_ref->{'EQMOD'}))
    {
      ...
      if($equip_name =~ m{^Wand \((.*)/(\d\d?)(st|rd|th) level caster\)})
      {
        ...
        if(exists $Spells_For_EQMOD{$spell_name})
        {
          ...
          mywarn qq(=> $equip_name: removing "COST" and adding "$eqmod_tag"),
            $file_for_error, $line_for_error;
        }
        else
        {
          mywarn qq(!!! $equip_name: not enough information to add charges),
            $file_for_error, $line_for_error;
        }
      }
      ...
      exists $line_ref->{'WEIGHT'}))
  {
    ...
    {
      $age = $line_ref->{'AGE'}[0];
      mywarn qq(=>Removing "$line_ref->{'AGE'}[0]"),
        $file_for_error, $line_for_error;
      delete $line_ref->{'AGE'};
    }
    if($line_ref->{'HEIGHT'})
    ...
    $height = $line_ref->{'HEIGHT'}[0];
      mywarn qq(=>Removing "$line_ref->{'HEIGHT'}[0]"),
        $file_for_error, $line_for_error;
    ...
    $weight = $line_ref->{'WEIGHT'}[0];
      mywarn qq(=>Removing "$line_ref->{'WEIGHT'}[0]"),
        $file_for_error, $line_for_error;
      delete $line_ref->{'WEIGHT'};
    }

    record_bioset_tags($dir, $race, $age, $height, $weight, $file_for_error, $line_for_error);
  }
     ...
     $line_info->[0] eq 'CLASS')
  {
    ...
    for (values %{$line_ref->{'SPELLTYPE'}})
    {
      my $spelltype = "";
      $spelltype = (/SPELLTYPE:(.*)/)[0];
      next if $spelltype eq "" or lc($spelltype) eq "Any";
     ...
     $source_curent_file ne $file_for_error)
  {
    ...
    if(exists $source_tags{File::Basename::dirname($file_for_error)})
    {
      ...
      # the directory .PCC
      my %line_tokens;
      while (my ($tag, $value) = each %{$source_tags{File::Basename::dirname($file_for_error)}})
      {
    ...
    {
      mywarn "No PCC source information found",
        $file_for_error, $line_for_error;
    }
  }
  ...
  if($conversion_enable{'Export lists'})
  {
    ...
    if($filetype eq 'SPELL')
    {
      ...
      # Write to file
      print SPELL qq("$spellname","$sourcepage","$line_for_error","$filename"\n);
    }

    if($filetype eq 'DOMAIN')
    {
      print DOMAIN qq("$line_ref->{'000DomainName'}[0]","$line_for_error","$filename"\n);
    }

    if($filetype eq 'CLASS')
    {
      my $class = ($line_ref->{'000ClassName'}[0] =~ /^CLASS:(.*)/)[0];
      print CLASS qq("$class","$line_for_error","$filename"\n)
    ...
    if($filetype eq 'EQUIPMENT')
    {
      ...
      my $outputname = "";
      $outputname = substr($line_ref->{'OUTPUTNAME'}[0],11) if exists $line_ref->{'OUTPUTNAME'};
      ...
      print EQUIPMENT qq("$equipname","$outputname","$line_for_error","$filename"\n);
    }
    ...
    if($filetype eq 'EQUIPMOD')
    {
      ...
      print EQUIPMOD qq("$equipmodname","$key","$type","$line_for_error","$filename"\n);
    }

    if($filetype eq 'FEAT')
    {
      my $featname = $line_ref->{$master_order{$filetype}[0]}[0];
      print FEAT qq("$featname","$line_for_error","$filename"\n);
    }

    if($filetype eq 'LANGUAGE')
    {
      my $languagename = $line_ref->{$master_order{$filetype}[0]}[0];
      print LANGUAGE qq("$languagename","$line_for_error","$filename"\n);
    }

    if($filetype eq 'SKILL')
    {
      my $skillname = $line_ref->{$master_order{$filetype}[0]}[0];
      print SKILL qq("$skillname","$line_for_error","$filename"\n);
    }
  }
  ...
  if($conversion_enable{'Generate BONUS and PRExxx report'})
  {
    foreach my $tagtype (sort keys %$line_ref)
    {
      if($tagtype =~ /^BONUS|^!?PRE/)
      {
        $bonus_prexxx_tag_report{$filetype}{$_} = 1
          for (@{$line_ref->{$tagtype}});
      }
    }
  }
...
#            else
#            {
#              mywarn qq(!! No level were given for "$_" found in "$lines_ref->[$i][1]{'CLASSES'}[0]"),
#                $filename,$i;
#            }
#          }
#
##          mywarn qq(**** $spellname: $_),$filename,$i for @classes_by_level;
#        }
#
...
#
#          mywarn qq(**** $spellname: $_),$filename,$i for @domains_by_level;
#        }
#      }
#    }
#  }
      ...
      for(my $i=0; $i < @{$lines_ref}; $i++)
      {
           ...
           exists $valid_line_type{$lines_ref->[$i][0]})
        {
          ...
          for(; $j < @{$lines_ref}; $j++)
          {
            ...
            next if ref($lines_ref->[$j]) ne 'ARRAY' ||
          ...
          if(keys %new_line > 1)
          {
            push @new_lines, [ $curent_linetype,
                               \%new_line,
                               $last_main_line,
                               $entity_name,
                               $line_info,
  ...
  if($conversion_enable{'CLASSSPELL convertion to SPELL'})
  {
    if($filetype eq 'CLASSSPELL')
    {
      ...
      mywarn qq(!! Already found a CLASSSPELL file in $dir),
        $filename
      ...
      for my $line (@$lines_ref)
      {
        ...
        next if ref($line) ne 'HASH' || (!exists $line->{'CLASS'} && !exists $line->{'000ClassSpellLevel'});
        ...
        if(exists $line->{'CLASS'})
        {
          ...
          {
            $curent_type = 2;
            mywarn qq(!!Don\'t know if "$curent_name" is a CLASS or a DOMAIN),
              $filename, $line_number;
          }
        }
        else
        {
          next if $curent_type == 2 || !exists $line->{'001ClassSpells'};

          # We store the CLASS name and Level

          for my $spellname (split '\|',$line->{'001ClassSpells'}[0])
          {
            push @{$class_spell{$dir}{$spellname}[$curent_type]{$line->{'000ClassSpellLevel'}[0]}},
              $curent_name;
    ...
    elsif($filetype eq 'SPELL')
    {
      ...
      if(exists $class_spell{$dir})
      {
        ...
        for my $line (@$lines_ref)
        {
          next if ref($line) ne 'ARRAY' || $line->[0] ne 'SPELL';
          $_ = $line->[1];

          next if ref ne 'HASH' || !exists $_->{'000SpellName'};
          ...
          if(exists $class_spell{$dir}{$spellname})
          {
            if(defined $class_spell{$dir}{$spellname}[0])
            {
              ...
              if(exists $_->{'CLASSES'})
              {
                mywarn qq(!! The is already a CLASSES tag for "$spellname"),
                  $filename, $line_number;
              }
              else
              {
                my @new_levels;
                for my $level (sort {$a <=> $b} keys %{$class_spell{$dir}{$spellname}[0]})
                {
                  my $new_level = join ',',@{$class_spell{$dir}{$spellname}[0]{$level}};
                  push @new_levels, "$new_level=$level";
                }
                my $new_classes = 'CLASSES:' . join '|', @new_levels;
                $_->{'CLASSES'} = [ $new_classes ];

                mywarn qq(=> SPELL $spellname: adding "$new_classes"),
                  $filename, $line_number;
              }
            }
            ...
            if(defined $class_spell{$dir}{$spellname}[1])
            {
              ...
              if(exists $_->{'DOMAINS'})
              {
                mywarn qq(!! The is already a DOMAINS tag for "$spellname"),
                  $filename, $line_number;
              }
              else
              {
                my @new_levels;
                for my $level (sort {$a <=> $b} keys %{$class_spell{$dir}{$spellname}[1]})
                {
                  my $new_level = join ',',@{$class_spell{$dir}{$spellname}[1]{$level}};
                  push @new_levels, "$new_level=$level";
                }
                my $new_domains = 'DOMAINS:' . join '|', @new_levels;
                $_->{'DOMAINS'} = [ $new_domains ];

                mywarn qq(=> SPELL $spellname: adding "$new_domains"),
                  $filename, $line_number;
              }
            }
        ...
        if(keys %{$class_spell{$dir}})
        {
          ...
          push @$lines_ref, "", "###Block:SPELL.MOD generated from the old CLASSSPELL files";
          ...
          for my $spellname (sort keys %{$class_spell{$dir}})
          {
            ...
            if(defined $class_spell{$dir}{$spellname}[0])
            {
              ...
              mywarn qq(=> SPELL $spellname.MOD: adding "$new_classes"),
                $filename, $line_number;
            ...
            if(defined $class_spell{$dir}{$spellname}[1])
            {
              # New DOMAINS
                ...
                mywarn qq(=> SPELL $spellname.MOD: adding "$new_domains"),
                  $filename, $line_number;

            }

            push @$lines_ref, [ 'SPELL',
                                \%newline,
                                1 + @$lines_ref,
                                $spellname,
        ...
        if(keys %new_spell_line)
        {
             ...
             !exists $new_spell_line{'BONUS:CASTERLEVEL'})
          {
            ...
            if(exists $new_spell_line{'ITEMCREATE'})
            {
              ...
              mywarn "!! Can't add BONUS:CASTERLEVEL for class \"$class\", " .
            ...
            else
            {
              ...
              mywarn qq(=> Adding missing "BONUS:CASTERLEVEL|$class|CL"),
                $filename;
            }
          }
  ...
  if($conversion_enable{'CLASSSKILL convertion to CLASS'})
  {
    if($filetype eq 'CLASSSKILL')
    {
      ...
      for (@$lines_ref)
      {
        ...
        next if ref ne 'HASH' || !exists $_->{'000ClassName'} || !exists $_->{'001SkillName'};
    ...
    elsif($filetype eq 'CLASS')
    {
      ...
      if(exists $class_skill{$dir})
      {
        ...
        for (@$lines_ref)
        {
          ...
          next unless ref($line_tokens) eq 'HASH' && exists $line_tokens->{'000ClassName'};
          ...
          if (exists $class_skill{$dir}{$class})
          {
            ...
            mywarn qq(=> Adding line "CLASS:$class\tCSKILL:$newskills"), $filename;
          }
        } continue { $index++ };
        ...
        if(exists $class_skill{$dir})
        {
          for (sort keys %{$class_skill{$dir}})
          {
            ...
            mywarn qq(=> Adding line "CLASS:$_.MOD\tCSKILL:$newskills"), $filename;
          }
        }
      }
    }
...
sub mylength
{
  ...
  if($beforelast > -1)
  {
    ...
    foreach my $subtag (@list[0..$beforelast])
    {
      ...
      for (@{$line_ref->{$tag}})
      {
        if(/:\.?CLEAR.?([^(]*)/)
        {
          ...
          if($1 ne "")
          {
            ...
            mywarn qq("$tag:$1" found before "$_"), $file_for_error, $line_for_error
              if exists $value_found{$1};
          }
          else
          {
            # Let's check if any value was found before
            mywarn qq("$tag" tag found before "$_"), $file_for_error, $line_for_error
              if keys %value_found;
          }
        }
        elsif(/:([^(]*)/)
        ...
        {
          mywarn "!! Didn't anticipate this tag: $_", $file_for_error, $line_for_error;
        }
      }
    }
    else
...
# -----------------
#
# Generate a list of sorted tags corresponding to the order
# of each line. Each class is considered it's own file.
#
# The array to be reformatted will be split in two arrays:
# The first line and the rest.
#
# Parameters :  $lines_ref          Reference to an array of lines
#                                   each line is a hash if not
#                                   already processed
#               $file_for_error
#               $line_for_error
#
# Returns:      An array of line after processed.

sub order_class_level(\@$$)
{
  # We do nothing if the CLASS Level are not to be rewritten
  return unless $writefiletype{'CLASS Level'};

  my ($lines_ref, $file_for_error, $line_for_error) = @_;
  my %taglength;

  # Let's find out the max length for each of the TAGS used.
  foreach my $line (@$lines_ref)
  {
    next unless ref($line) eq "HASH"; # We only check the lines with tags.

    # Local copy (we do not want to modify the original
    my %line = %$line;

    # We check each tag to find the maximum length used in the file
    foreach my $tag (keys %line)
    {
      my $Length = mylength($line{$tag});

      $taglength{$tag} = $Length if !exists $taglength{$tag} || $taglength{$tag} < $Length;
    }
  }

  # Here we find the order of the tags for the new lines.
  my %seen;
  my @tagorder;
  foreach my $tag (@{$master_order{'CLASS Level'}})
  {
    if(exists $taglength{$tag})
    {
      push @tagorder, $tag;
      $seen{$tag}++;
    }
  ...
  # Any tags that was not in the master order list goes at the end.
  foreach my $tag (keys %taglength)
  {
    if(!$seen{$tag})
    {
      push @tagorder, $tag;
  ...
  # We ajust the length of each column to match a product of tablength
  foreach my $tag (keys %taglength)
  {
    # We also add an extra $tablength to mark the separation between column.
    # That way, there will be at least one full tab between each column.
    if($taglength{$tag} % $tablength)
    {
      $taglength{$tag} += $tablength - $taglength{$tag} % $tablength;
    }
    else
    {
      # The length is already an exact tabulation, we just have to add a new tab.
      $taglength{$tag} += $tablength;
    }
  }

  # We will now modify each line to put them in the right order and pad them with
  # the proper number of tabs.

  foreach my $line (@$lines_ref)
  {
    next unless ref($line) eq 'HASH';

    my $newline = "";       # The new line that will replace the Hash.
    ...
    foreach my $tag (@tagorder)
    {
      ...
      {
        my $value = "";
        # It might be a list of columns
        if(ref($line->{$tag}) eq 'ARRAY')
        {
###### remove the sort for now
###          $value = join "\t", sort @{$line->{$tag}};
          $value = join "\t", @{$line->{$tag}};
        }
        else
        {
        ...
        my $space_to_add = $taglength{$tag} - mylength($line->{$tag});
        my $tab_to_add = int($space_to_add / $tablength) + ($space_to_add % $tablength ? 1 : 0);
        $newline .= $value . "\t" x $tab_to_add;
      }
      else
      {
        # The tag was not present on that line, we add empty space.
        $newline .= "\t" x int($taglength{$tag} / $tablength);
      }
    }

    # We replace the Hash reference with the new line
    $line = $newline;
  }
...
# ------------
#
# Change the @data and relative paths found in the .lst for
# the real thing.

sub findfullpath($$$)
{
  my ($file,$currentbasedir,$basepath) = @_;
  ...
  # Replace @ by the basedir or add the currentbasedir to the filename.
  $file = "$currentbasedir/$file" unless ($file =~ s/^\@/$basepath/i);
  $file;
}
...
sub report_tag_sort()
{
  ...
  my $not_left = $left =~ s/^!//;
  my $not_right = $right =~ s/^!//;

  $left cmp $right || $not_left cmp $not_right;
...
sub embedded_coma_split($;$)
{
  ...
  # Now we can split
  return map { s/&coma;/,/g; $_ } split $separator, $newlist
}

###############################################################
###############################################################
###
### Closure for mywarn, set_mywarn_filename and
### set_mywarn_header functions.
###
...
# ------
#
# Generate a warning message with the filename an line number.
# if available.

sub mywarn($$;$)
{
  # We call ewarn with the INFO level.
  ewarn(NOTICE,@_[0],@_[1],@_[2]);

#    my ($message, $filename, $linenumber) = (@_,undef);
#
#    # Verify if it's a warning message and if warnings must be displayed
#    return if($cl_options{nowarning} && $message =~ /^\*\*/);
#
...
# -----
#
# Enhance mywarn. It accept an extra parameter telling the
# level of the message.
#
# Possible levels:
#
#    DEBUG            Normal message + debug message for the programmer
...
#   $filename         Name of the file where the error is found
#   $linenumber       Line number where the error is found (optionnal)

sub ewarn ($$$;$)
{
  my ($level,$message,$filename,$linenumber) = (@_,undef);
...
# -------------------
#
# Replace the curent $previousfilename with a specific value.

sub set_mywarn_filename($)
{
  $previousfilename = $_[0];
}
...
# $IsFirstError to make the header display on the first
# mywarn call for this header.

sub set_mywarn_header($)
{
  $IsFirstError = 1;
  $header = $_[0];
  $header = "\n" . $header unless $IsFirstLine;
  set_mywarn_filename('');
}
...
###
### End of the mywarn, set_mywarn_filename and
### set_mywarn_header closure.
###
###############################################################
###############################################################
...
# Moving this out of the bigin as a workaround for bug
...
#             $height           HEIGHT tag
#             $weight           WEIGHT tag
#             $file_for_error   To use with mywarn
#             $line_for_error   To use with mywarn

sub record_bioset_tags($$$$$$$)
{
  my ($dir, $race, $age, $height, $weight,
      $file_for_error,$line_for_error) = @_;

  # Check to see if default apply
  for (keys %DefaultBioset)
  {
    if(index($race, $_) == 0)
    {
      # The race name is included in the default
      # We now verify the values
      $age    = "" if $DefaultBioset{$_}[0] eq $age;
      $height = "" if $DefaultBioset{$_}[1] eq $height;
      $weight = "" if $DefaultBioset{$_}[2] eq $weight;
      last;
    }
  }
  ...
  if($age)
  {
    ...
    {
      mywarn qq(BIOSET generation: There is already a AGE tag recorded for a race named "$race" in this directory.),
        $file_for_error, $line_for_error;
    }
    else
    {
  ...
  if($height)
  {
    ...
    {
      mywarn qq(BIOSET generation: There is already a HEIGHT tag recorded for a race named "$race" in this directory.),
        $file_for_error, $line_for_error;
    }
    else
    {
  ...
  if($weight)
  {
    ...
    {
      mywarn qq(BIOSET generation: There is already a WEIGHT tag recorded for a race named "$race" in this directory.),
        $file_for_error, $line_for_error;
    }
    else
    {
...
sub generate_bioset_files()
{
  for my $dir (sort keys %RecordedBiosetTags)
  {
    ...
    $filename =~ s/$opt_inputpath/$opt_outputpath/i;

    open BIOSET, ">$filename" or die "Can't create $filename: $!";

    # Printing the name of the new file generated
    print STDERR $filename, "\n";

    # Header part.
    print BIOSET <<END_OF_HEADER;
    ...
    for my $racename (sort keys %{$RecordedBiosetTags{$dir}})
    {
         ...
         exists $RecordedBiosetTags{$dir}{$racename}{WEIGHT})
      {
        ...
        my ($m_ht_min, $m_ht_dice, $m_ht_sides, $m_ht_bonus,
          ...
          = (split ':', $RecordedBiosetTags{$dir}{$racename}{HEIGHT})[2,3,4,5,7,8,9,10];

        my ($m_wt_min, $m_wt_dice, $m_wt_sides,
            $f_wt_min, $f_wt_dice, $f_wt_sides)
          = (split ':', $RecordedBiosetTags{$dir}{$racename}{WEIGHT})[2,3,4,6,7,8];
        ...
        $height_weight_line .= 'SEX:Male[BASEHT:' . $m_ht_min . '|HTDIEROLL:' . $m_ht_dice . 'd' . $m_ht_sides;
        $height_weight_line .= '+' . $m_ht_bonus if $m_ht_bonus > 0;
        $height_weight_line .= $m_ht_bonus if $m_ht_bonus < 0;

        # Male weight caculation
        $height_weight_line .= '|BASEWT:' . $m_wt_min . '|WTDIEROLL:' . $m_wt_dice . 'd' . $m_wt_sides;
        $height_weight_line .= '|TOTALWT:BASEWT+(HTDIEROLL*WTDIEROLL)]';

        # Female height caculation
        $height_weight_line .= 'Female[BASEHT:' . $f_ht_min . '|HTDIEROLL:' . $f_ht_dice . 'd' . $f_ht_sides;
        $height_weight_line .= '+' . $f_ht_bonus if $f_ht_bonus > 0;
        $height_weight_line .= $f_ht_bonus if $f_ht_bonus < 0;

        # Female weight caculation
        $height_weight_line .= '|BASEWT:' . $f_wt_min . '|WTDIEROLL:' . $f_wt_dice . 'd' . $f_wt_sides;
        $height_weight_line .= '|TOTALWT:BASEWT+(HTDIEROLL*WTDIEROLL)]';
      }
      ...
      if(exists $RecordedBiosetTags{$dir}{$racename}{AGE})
      {
        ...
        $age_line = '### Old tag for race ' . $racename . '=> ' . $RecordedBiosetTags{$dir}{$racename}{AGE};
      }

      print BIOSET $height_weight_line, "\n"  if $height_weight_line;
      print BIOSET $age_line, "\n"            if $age_line;
#      print BIOSET "\n";
    }

    close BIOSET or die "Can't close $filename: $!";
  }
}
...
sub generate_css($)
{
  ...
  open(CSSFILE, ">$newfile") or warn "Can't create $newfile: $!\n";

  print CSSFILE <<EOFCSS;
BODY {
...
EOFCSS
  ...
  close(CSSFILE) or warn "Can't close $newfile: $!\n";
}
...
I'm using ActivePerl v5.8.4 (build 810) but any standard distribution with version 5.5 and
...
=head1 SYNOPSIS
  ...
  # the warning message (prefixed with **) will not be outputed.
  perl prettylst.pl -noxcheck -nowarning -inputpath=<PATH> -outputerror=<ERROR_FILE>
  perl prettylst.pl -nx -nw -i=<PATH> -e=<ERROR_FILE>
...
B<This option is now on be default>
...
=item B<-nowarning> or B<-nw>

Suppress the printout of warning messages i.e. the messages that are prefixed with **.
...
=item * prettylst-release-notes-134.html
...
Copyright 2002 to 2005 by E<Eacute>ric E<quot>Space MonkeyE<quot> Beaudoin -- <mailto:beaudoer@videotron.ca>
...
New set_mywarn_header function
