...
public class BonusManager
{
	...
	void buildActiveBonusMap()
	{
		...
		for (BonusObj bonus : bonusListCopy)
		{
			...
			for (BonusPair bp : getStringListFromBonus(bonus))
			{
				...
				setActiveBonusStack(iBonus, bp.bonusKey,
					nonStackMap, stackMap);
				totalBonusesForType(nonStackMap, stackMap,
					bp.bonusKey, activeBonusMap);
		...
		Map<String, String> nonStackMap, Map<String, String> stackMap)
	{
		...
		{
			final double iBonus = bp.resolve(pc).doubleValue();
			setActiveBonusStack(iBonus, bp.bonusKey, nonStackMap, stackMap);
			totalBonusesForType(nonStackMap, stackMap,
				bp.bonusKey, activeBonusMap);
	...
	private void setActiveBonusStack(double bonus,
		String bonusType, Map<String, String> nonStackbonusMap,
		Map<String, String> stackingBonusMap)
	{
		...
		{
			final String aVal = nonStackbonusMap.get(bonusType);

			if (aVal == null)
			{
				putActiveBonusMap(bonusType, String.valueOf(bonus), nonStackbonusMap);
			}
			else
			{
		...
		// a stacking bonus
		{
			final String aVal = stackingBonusMap.get(bonusType);

			if (aVal == null)
			{
				putActiveBonusMap(bonusType,
					...
					for (BonusPair bp : getStringListFromBonus(bonus))
					{
						if (bp.bonusKey.startsWith(prefix))
						{
							setActiveBonusStack(bp.resolve(pc).doubleValue(),
									bp.bonusKey, nonStackMap, stackMap);
							totalBonusesForType(nonStackMap, stackMap,
