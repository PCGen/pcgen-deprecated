...
;
; Known issues
;     PCGen install directory is not romoved under full uninstall
;
;
; Version history
...
!define OverVer "1.5"
!define OutName "setup_1.2_pcgen5713_win"
...
Section "PCGen" Section1
  ...
  SetOutPath "$INSTDIR\\${APPDIR}\\${APPDIR}\"
...
SubSection "Alpha"
  ...
  SetOutPath "$INSTDIR\\${APPDIR}\\${APPDIR}\data\alpha\alderacentgroup\"
  ...
  SetOutPath "$INSTDIR\\${APPDIR}\\${APPDIR}\data\alpha\avalanchepress\"
  ...
  SetOutPath "$INSTDIR\\${APPDIR}\\${APPDIR}\data\alpha\bastionpress\"
  ...
  SetOutPath "$INSTDIR\\${APPDIR}\\${APPDIR}\data\alpha\doghouserules\"
  ...
  SetOutPath "$INSTDIR\\${APPDIR}\\${APPDIR}\data\alpha\fantasyflightgames\"
...
Section "-Local" Section4
  ...
  CreateShortCut "$DESKTOP\${APPDIR}.lnk" "$INSTDIR\\${APPDIR}\pcgen_high_mem.bat"
  CreateShortCut "$SMPROGRAMS\PCGen\${APPDIR}\${APPDIR}-High.lnk" "$INSTDIR\\${APPDIR}\pcgen_high_mem.bat"
  CreateShortCut "$SMPROGRAMS\PCGen\${APPDIR}\${APPDIR}.lnk" "$INSTDIR\\${APPDIR}\pcgen.bat"
  CreateShortCut "$SMPROGRAMS\PCGen\${APPDIR}\Release Notes.lnk" "$INSTDIR\\${APPDIR}\pcgen-release-notes-5713.html"
  CreateShortCut "$SMPROGRAMS\PCGen\${APPDIR}\News.lnk" "http://pcgen.sourceforge.net/01_news.php"
  CreateShortCut "$SMPROGRAMS\PCGen\${APPDIR}\uninstall-${APPNAME}.lnk" "$INSTDIR\\${APPDIR}\uninstall.exe"
...
Section Uninstall
  ...
  ; Delete self
  Delete "$INSTDIR\\${APPDIR}\PCGen\${APPDIR}\uninstall.exe"
