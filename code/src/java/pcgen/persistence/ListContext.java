...
public class ListContext
{
	...
	private final TrackingListCommitStrategy edits =
			new TrackingListCommitStrategy();
			...
			for (OwnerURI ou : edits.positiveMasterMap.getSecondaryKeySet(list))
			{
					...
					.getTertiaryKeySet(list, ou))
				{
					AssociatedPrereqObject assoc =
							edits.positiveMasterMap.get(list, ou, child);
					AssociatedPrereqObject edge =
							commit.addToMasterList(assoc
		...
		CDOMObject neg, CDOMReference<CDOMList<BT>> list)
	{
		...
		for (CDOMReference<BT> ref : mods)
		{
			for (AssociatedPrereqObject assoc : neg.getListAssociations(list,
				ref))
			{
				commit.removeFromList(assoc
					.getAssociation(AssociationKey.TOKEN), owner, list, ref);
			}
		}
	}
		...
		CDOMReference<CDOMList<BT>> list)
	{
		...
		for (CDOMReference<BT> ref : mods)
		{
				...
				ref))
			{
				...
				AssociatedPrereqObject edge =
						commit.addToList(token, owner, list, ref);
				Collection<AssociationKey<?>> associationKeys =
						assoc.getAssociationKeys();
				for (AssociationKey<?> ak : associationKeys)
				{
	...
	public class TrackingListCommitStrategy implements ListCommitStrategy
	{
		...
		private TripleKeyMap<CDOMReference<? extends CDOMList<?>>, OwnerURI, LSTWriteable, AssociatedPrereqObject> positiveMasterMap =
				new TripleKeyMap<CDOMReference<? extends CDOMList<?>>, OwnerURI, LSTWriteable, AssociatedPrereqObject>();
			...
			CDOMReference<? extends CDOMList<?>> swl)
		{
			...
			Set<LSTWriteable> added =
					positiveMasterMap.getTertiaryKeySet(swl, lo);
