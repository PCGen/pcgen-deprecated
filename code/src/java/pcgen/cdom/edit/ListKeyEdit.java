 ...
 * 
 * ListKeyEdit is an UndoableEdit which represents the atomic deletion of a Node
 * from a Graph.
 * 
 ...
 */
public class ListKeyEdit<T> extends AbstractUndoableEdit implements
		UndoableEdit
{
	...
	/**
	 * Represents an Edit of a ListKey on a CDOMObject.
	 * 
	 * The CDOMObject is passed by Reference into ListKeyEdit, and ListKeyEdit
	 * therefore maintains a reference to this object. ListKeyEdit then agrees
	 * ("by contract") that it will only (a) call appropriate methods in
	 * CDOMObject interface.
	 * 
	 ...
	 */
	public ListKeyEdit(String editPresentationName, CDOMObject cdomObject,
		ListKey<T> key, T newVal, boolean wasAdded)
	{
		if (cdomObject == null)
		{
	...
	public void redo()
	{
		...
		if (added)
		{
			cdo.addToListFor(ListKey, newValue);
		}
		else
		{
			cdo.removeFromListFor(ListKey, newValue);
		}
	}
