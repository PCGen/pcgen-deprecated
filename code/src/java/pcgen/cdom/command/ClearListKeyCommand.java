...
public class ClearListKeyCommand<T> implements CDOMCommand
{
	...
	{
		List<T> list = cdo.getListFor(listKey);
		if (list == null)
		{
			return new InsignificantUndoableEdit();
		}
		cdo.removeListFor(listKey);
		CompoundEdit edit = new CompoundEdit();
		// TODO Does this need to iterate BACKWARDS to preserve order??
		for (T obj : list)
		{
			edit.addEdit(new ListKeyEdit<T>(name, cdo, listKey, obj, false));
		}
		edit.end();
		return edit;
	}
