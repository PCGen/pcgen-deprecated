...
public class GroupReferenceContext
{
	...
	public <T extends CDOMObject> CDOMGroupRef<T> getCDOMTypeReference(Class<T> c,
		String... val)
	{
		if (val.length == 0)
		{
		...
		for (String s : val)
		{
			...
			{
				throw new IllegalArgumentException(
					"Cannot build Reference with type conaining a period: " + s);
			}
			else if (s.indexOf('=') != -1)
			{
				throw new IllegalArgumentException(
					"Cannot build Reference with type conaining an equals: " + s);
			}
		}
		Arrays.sort(val);
	...
	public <T extends CategorizedCDOMObject<T>> CDOMGroupRef<T> getCategorizedCDOMTypeReference(
	...
	public <T extends CategorizedCDOMObject<T>> CDOMGroupRef<T> getCategorizedCDOMAllReference(
