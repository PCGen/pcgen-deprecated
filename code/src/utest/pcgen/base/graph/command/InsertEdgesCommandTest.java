...
import pcgen.base.graph.command.InsertEdgesCommand;
 ...
 */
public class InsertEdgesCommandTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		factory = new InsertEdgesCommand("edit", graph, Arrays.asList(edge));
		factory2 = new InsertEdgesCommand("edit", graph, Arrays.asList(edge2));
		factory3 = new InsertEdgesCommand("edit", graph, Arrays.asList(edge3,
				edge4));
		factory4 = new InsertEdgesCommand("edit", graph, Arrays.asList(edge4));
	}
	...
	public void testExecute() {
		...
		assertFalse(graph.containsEdge(innocentEdge));
		Command fac = new InsertEdgesCommand("edit", graph, Arrays
				.asList(hedge));
		fac.execute();
	...
	public void testGetPresentationName() {
		...
		assertEquals("Insert Graph Edges", new InsertEdgesCommand(null, graph,
				Arrays.asList(edge)).getPresentationName());
	}

	public void testGetInsertGraphEdgeEditor() {
		try {
			new InsertEdgesCommand("edit", null, Arrays.asList(edge));
			fail();
		} catch (IllegalArgumentException iae) {
			// OK
		}
		try {
			new InsertEdgesCommand("edit", graph, null);
			fail();
		} catch (IllegalArgumentException iae) {
			// OK
		}
		try {
			new InsertEdgesCommand("edit", graph, new ArrayList());
			fail();
		} catch (IllegalArgumentException iae) {
			// OK
		}
		new InsertEdgesCommand(null, graph, Arrays.asList(edge));
	}
