...
import pcgen.base.graph.command.DeleteNodesCommand;
 ...
 */
public class DeleteNodesCommandTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		factory = new DeleteNodesCommand("edit", graph, Arrays.asList(node));
		factory2 = new DeleteNodesCommand("edit", graph, Arrays.asList(node2,
				node3));
	}
	...
	public void testBadExecute() {
		try {
			new DeleteNodesCommand("edit", graph, Arrays
					.asList(new Integer(-1))).execute();
			fail();
	...
	public void testGetPresentationName() {
		...
		assertEquals("Delete Graph Nodes", new DeleteNodesCommand(null, graph,
				Arrays.asList(node)).getPresentationName());
	}

	public void testGetDeleteGraphNodeEditor() {
		try {
			new DeleteNodesCommand("edit", null, Arrays.asList(node));
			fail();
		} catch (IllegalArgumentException iae) {
			// OK
		}
		try {
			new DeleteNodesCommand("edit", graph, null);
			fail();
		} catch (IllegalArgumentException iae) {
			// OK
		}
		try {
			new DeleteNodesCommand("edit", graph, new ArrayList());
			fail();
		} catch (IllegalArgumentException iae) {
			// OK
		}
		new DeleteNodesCommand(null, graph, Arrays.asList(node));
	}
