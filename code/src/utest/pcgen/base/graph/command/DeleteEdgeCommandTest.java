...
import pcgen.base.graph.command.DeleteEdgeCommand;
 ...
 */
public class DeleteEdgeCommandTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		factory = new DeleteEdgeCommand("edit", graph, edge);
	}
	...
	public void testExecute() {
		...
		assertTrue(graph.containsEdge(innocentEdge));
		Command factory2 = new DeleteEdgeCommand("edit", graph, hedge);
		factory2.execute();
	...
	public void testBadExecute() {
		try {
			Edge hedge = new DefaultHyperEdge(Arrays.asList(edge.getNodeAt(0)));
			new DeleteEdgeCommand("edit", graph, hedge).execute();
			fail();
	...
	public void testGetPresentationName() {
		...
		assertEquals("Delete Graph Edge", new DeleteEdgeCommand(null, graph,
				edge).getPresentationName());
	}

	public void testGetDeleteGraphEdgeEditor() {
		try {
			new DeleteEdgeCommand("edit", null, edge);
			fail();
		} catch (IllegalArgumentException iae) {
			// OK
		}
		try {
			new DeleteEdgeCommand("edit", graph, null);
			fail();
		} catch (IllegalArgumentException iae) {
			// OK
		}
		new DeleteEdgeCommand(null, graph, edge);
	}
